...
GENERATED_BY = (
    ...
    "root_tie_rank_selected_null_simulation_pilot"
)
...
COMBINED_FEASIBILITY_ROWS_OUTPUT = (
    "root_tie_rank_selected_null_combined_feasibility_rows.csv"
)
COMBINED_FEASIBILITY_STRATA_OUTPUT = (
    "root_tie_rank_selected_null_combined_feasibility_strata.csv"
)
COMBINED_FEASIBILITY_SUMMARY_OUTPUT = (
    "root_tie_rank_selected_null_combined_feasibility_summary.csv"
)
...
def _json_default(value: object) -> object:
    if isinstance(value, RootTieRankSelectedNullSimulationConfig):
        return asdict(value)
                ...
                row.update(
                    {
                        ...
                        "null_seed": int(seed),
                        "null_feature_probability": float(
                            null_case["null_feature_probability"]
                        ),
                    }
                )
...
def build_target_support_rows(
    *,
    ...
    targets = combined_feasibility_rows[
        combined_feasibility_rows["case_id"].astype(str).isin(
        ...
        records.append(
            {
                ...
                "root_conditioning_stratum_key": str(
                    row["root_conditioning_stratum_key"]
                ),
                "root_sibling_selected_ratio": float(
                    row["root_sibling_selected_ratio"]
                ),
                "root_tie_rank_median_fraction": float(
                    row["root_tie_rank_median_fraction"]
                ),
                "root_edge_path_statistic_margin": float(
                ...
                "root_bandwidth_reopen_count": float(
                    row["root_bandwidth_reopen_count"]
                ),
                ...
                "calibration_feasibility_status": str(
                    row["calibration_feasibility_status"]
                ),
            }
        )
...
def evaluate_root_tie_rank_selected_null_simulation(
    ...
    null_mixed_with_strata = feasibility_rows[
        feasibility_rows["calibration_role"].astype(str).eq(
