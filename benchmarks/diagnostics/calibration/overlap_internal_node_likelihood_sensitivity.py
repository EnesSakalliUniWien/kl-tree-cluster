...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.overlap_internal_node_likelihood_sensitivity"
)
...
def apply_internal_node_thresholds(
    ...
    *,
    ...
    candidate = (
        ...
        & _numeric(rows, "continuous_context_min_margin").ge(
            float(thresholds.context_margin_floor)
        )
        ...
        & _numeric(rows, "edge_norm_balance").ge(
            float(thresholds.edge_norm_balance_floor)
        )
        & _numeric(rows, "fragment_risk_proxy_score").le(
            float(thresholds.fragment_risk_ceiling)
        )
    )
    ...
    return {
        ...
        "p_extreme_log_bayes_factor_floor": float(
            thresholds.p_extreme_log_bayes_factor_floor
        ),
...
def summarize_internal_node_likelihood_sensitivity(
    ...
    *,
    ...
    default_status = (
        str(default["candidate_status"].iloc[0]) if not default.empty else "missing"
    )
    summary = {
        "schema_version": SCHEMA_VERSION,
        "study_role": STUDY_ROLE,
...
def run_overlap_internal_node_likelihood_sensitivity(
    ...
    default_rule = InternalNodeLikelihoodThresholds(
        ...
        p_extreme_log_bayes_factor_floor=float(
            config.p_extreme_log_bayes_factor_floor
        ),
    )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--context-margin-floors", default=",".join(map(str, DEFAULT_CONTEXT_MARGIN_FLOORS)))
    parser.add_argument("--subspace-floors", default=",".join(map(str, DEFAULT_SUBSPACE_FLOORS)))
    parser.add_argument("--size-balance-floors", default=",".join(map(str, DEFAULT_SIZE_BALANCE_FLOORS)))
    parser.add_argument(
...
def main() -> None:
    ...
    run_overlap_internal_node_likelihood_sensitivity(
        OverlapInternalNodeLikelihoodSensitivityConfig(
            ...
            edge_norm_balance_floors=_parse_float_grid(
                str(args.edge_norm_balance_floors)
            ),
            fragment_risk_ceilings=_parse_float_grid(
                str(args.fragment_risk_ceilings)
            ),
        )
    )
