...
def analyze_covariance_laplacian(
    ...
    *,
    ...
    if float(correlation_threshold) < 0.0:
        raise ValueError(
            f"correlation_threshold must be non-negative; got {correlation_threshold!r}."
        )
    if float(min_algebraic_connectivity) < 0.0:
        raise ValueError(
            f"min_algebraic_connectivity must be non-negative; got {min_algebraic_connectivity!r}."
        )
    ...
    graph_density = (
        float(np.count_nonzero(edge_mask) / n_possible_edges) if n_possible_edges else 0.0
    )
    ...
    for matrix_id, covariance, metadata in matrices:
        ...
        rows.append(
            analyze_covariance_laplacian(
                covariance,
                matrix_id=matrix_id,
                matrix_context_role=str(metadata_dict.pop("matrix_context_role", "unspecified")),
