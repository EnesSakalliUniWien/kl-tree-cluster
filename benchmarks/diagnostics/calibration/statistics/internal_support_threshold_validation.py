...
def _threshold_failures(
    ...
    *,
    ...
    if float(row["leave_one_record_max_delta_log_c"]) > thresholds.max_leave_one_record_delta_log_c:
...
def summarize_internal_support_threshold_rows(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    for profile_id, group in rows.groupby("threshold_profile_id", sort=False):
        ...
        has_mixed_outcomes = (
            has_outcome_labels and bool(null_mask.any()) and bool(signal_mask.any())
        )
        summaries.append(
            {
                "threshold_profile_id": profile_id,
                ...
                "signal_retention_rate_among_admissible": (
                    float((split_rejected & admissible_signal).sum() / admissible_signal.sum())
