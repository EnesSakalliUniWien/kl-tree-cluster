"""Benchmark pooled versus within-child discrete sibling covariance models.
...
def _regularized_binary_probabilities(
    ...
    *,
    ...
    return (sample_size * probabilities + pseudo_count) / (sample_size + 2.0 * pseudo_count)
...
def _binary_within_z(
    ...
    *,
    ...
    within_variance = first_reg * (1.0 - first_reg) / float(first_sample_size) + second_reg * (
        1.0 - second_reg
    ...
    pooled_variance = (
        pooled * (1.0 - pooled) * (1.0 / float(first_sample_size) + 1.0 / float(second_sample_size))
    )
...
def _categorical_within_z(
    ...
    *,
    ...
    for block in feature_space.blocks:
        ...
        pooled = (
            float(first_sample_size) * first_prob + float(second_sample_size) * second_prob
        ) / (float(first_sample_size) + float(second_sample_size))
        covariance = (
        ...
        grouped.append(
            {
                **dict(zip(group_columns, keys, strict=True)),
                ...
                "min_within_jeffreys_bh": float(group["within_jeffreys_fixed_coordinate_bh"].min()),
                "min_guarded_25pct_bh": float(
                    group["guarded_within_25pct_pooled_fixed_coordinate_bh"].min()
                ),
                "median_current_neglog10_bh": float(group["current_pooled_neglog10_bh"].median()),
                "median_within_neglog10_bh": float(group["within_jeffreys_neglog10_bh"].median()),
