...
def same_z_rows_for_result(
    *,
    ...
    for parent in tree.nodes:
        ...
        parent_eigenvalues = spectral_context.principal_component_eigenvalues_by_node[parent]
        ...
        summary_rows.append(
            {
                ...
                "median_max_edge_neglog10_bh": float(group["max_edge_neglog10_bh"].median()),
                ...
                "median_fixed_global_neglog10": float(group["fixed_global_neglog10"].median()),
...
def plot_same_z_functional_decomposition(
    ...
    *,
    ...
    if not plotted.empty:
        max_identity = float(
            np.nanmax(plotted[["projected_sibling_neglog10", "max_edge_neglog10_bh"]].to_numpy())
        )
    ...
    for patch, color in zip(
        boxes["boxes"], ["#4c78a8", "#72b7b2", "#54a24b", "#e45756"], strict=False
    ):
    ...
    axes[1, 1].barh(
        class_counts.index.astype(str), class_counts.to_numpy(dtype=float), color="#4c78a8"
    )
...
def run_same_z_functional_decomposition(
    *,
    ...
    rows = pd.concat(all_rows, ignore_index=True) if all_rows else pd.DataFrame(columns=ROW_COLUMNS)
    summary = (
        summarize_same_z_rows(rows) if not rows.empty else pd.DataFrame(columns=SUMMARY_COLUMNS)
    )
        ...
        json.dumps(
            {
                ...
                "data_roles": [
                    NULL_OUTPUT_ROLE if role == "null" else str(role) for role in data_roles
                ],
