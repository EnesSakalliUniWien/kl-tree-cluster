...
GENERATED_BY = "benchmarks.diagnostics.calibration.statistics.null_law_decomposition_panel"
PROJECTION_SOURCES = (
...
def _random_orthonormal_projection(
    *,
    ...
    if projection_dimension > width:
        raise ValueError(f"projection_dimension={projection_dimension} exceeds width={width}.")
...
def _rows_for_projection_source(
    *,
    ...
    tree, (projection_dimensions, parent_projections, parent_eigenvalues) = _projection_context(
        tree_data=tree_data,
        test_data=test_data,
        feature_space=feature_space,
        ...
        rows.append(
            {
                ...
                "projection_orthonormality_error": projection_orthonormality_error(projection),
...
def _null_law_status(
    ...
    *,
    ...
    if projection_source == "adaptive_same_sample" and tail_rate > float(alpha) + float(tolerance):
    ...
    for case in _select_cases(suite=config.suite, case_names=config.case_names):
        ...
        for replicate in range(int(config.replicates)):
            for edge_alpha in config.edge_alphas:
                data_seed = (
                    int(config.base_seed) + replicate * 1009 + int(float(edge_alpha) * 1_000_000)
                )
