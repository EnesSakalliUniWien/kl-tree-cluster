...
def normalize_statistic_distribution_records(records: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Statistic distribution panel is missing columns: {sorted(missing)!r}.")
    table = records.copy()
    ...
    rows = pd.DataFrame(
        {
            ...
            "chi_square_centered_residual": (statistic - degrees) / np.sqrt(2.0 * degrees),
...
def _shape_status(
    *,
    ...
    tail_aligned = math.isfinite(alpha_tail_rate) and abs(alpha_tail_rate - alpha) <= tail_tolerance
    skew_aligned = (
...
def summarize_statistic_distribution_shape(
    ...
    *,
    ...
    for key, group in rows.groupby(list(group_columns), dropna=False, sort=True):
        ...
        alternate_expected_skew = _expected_chi_square_mixture_skew(alternate_degrees.dropna())
        ...
        row.update(
            {
                ...
                "alternate_skew_excess_over_df_reference": (
                    ...
                    if math.isfinite(empirical_skew) and math.isfinite(alternate_expected_skew)
                    else math.nan
                ),
                "alternate_chi_square_p_value_q50": (
                    float(alternate_p_values.quantile(0.50)) if n_alternate_rows else math.nan
                ),
                "alternate_chi_square_p_value_q05": (
                    float(alternate_p_values.quantile(0.05)) if n_alternate_rows else math.nan
                ),
