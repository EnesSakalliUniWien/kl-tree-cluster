...
GENERATED_BY = (
    ...
    "root_tie_rank_spectral_lift_parameter_sweep"
)
    ...
    *,
    ...
    if family in {
        ...
        matrix, metadata = generate_coherent_rank_one_spike_matrix(
            base_case=base_case,
            seed=int(seed),
            spike_feature_fraction=(
                spike_fraction if math.isfinite(spike_fraction) else 0.1
            ),
                    ...
                    row.update(
                        {
                            ...
                            "calibration_role": (
                                "diagnostic_proposal_not_null_support"
                            ),
                            "replicate": int(replicate),
                            "proposal_family": family,
                            "proposal_calibration_status": (
                                "diagnostic_proposal_not_calibration"
                            ),
                            "null_seed": int(seed),
...
def evaluate_spectral_lift_parameter_sweep(
    ...
    conditioned_requested = (
        CONDITIONED_COHERENT_RANK_ONE_SPIKE_PROPOSAL in config.proposal_families
    )
