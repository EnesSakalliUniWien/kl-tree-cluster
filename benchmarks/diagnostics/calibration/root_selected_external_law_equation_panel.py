...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.root_selected_external_law_equation_panel"
)
...
def build_root_selected_external_law_equation_rows(
    *,
    ...
    for _, target_row in external_law_target_rows.sort_values("target_case_id").iterrows():
        ...
        needs_new_spectral_support = bool(
            math.isfinite(gap_to_hull) and gap_to_hull > 1e-9
        )
        moment_feasible = (
        ...
        if target_status == "exact_tail_support_available":
            ...
            next_step = (
                "sample_or_derive_external_law_with_positive_mass_on_tail_event"
            )
