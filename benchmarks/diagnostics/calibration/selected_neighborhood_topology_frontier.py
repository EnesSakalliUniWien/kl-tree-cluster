...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.selected_neighborhood_topology_frontier"
)
...
def _root_margin_status(
    *,
    ...
    law_status = str(
        margin.get("root_selected_region_law_status", "root_margin_status_missing")
    )
    return (
        law_status,
...
def build_topology_frontier_rows(
    ...
    *,
    ...
    for _, row in measurability_rows.iterrows():
        ...
        effective_support_pass = (
            math.isfinite(effective_support)
            and effective_support >= float(effective_support_floor)
        )
        best_case_tau = _finite_float(
            row.get("interpolation_best_case_required_tau_s_for_alpha", math.nan)
        )
        bandwidth_reference_reopens = (
            math.isfinite(best_case_tau)
            and best_case_tau <= float(reference_tau_s)
        )
        bandwidth_reference_direct_positive_reopens = (
        ...
        records.append(
            {
                ...
                "bandwidth_reference_reopen_count": _bool_sum(
                    group["bandwidth_reference_reopens"]
                ),
                ...
                "root_structural_proxy_pass_count": _bool_sum(
                    root["root_structural_proxy_pass"]
                ),
                ...
                "nonroot_near_frontier_count": _bool_sum(
                    nonroot["nonroot_near_frontier"]
                ),
                ...
                "hybrid_strict_support_count": _bool_sum(
                    non_direct["hybrid_strict_support"]
                ),
                "median_root_outgoing_balance": _finite_median(
                    root["root_outgoing_balance"]
                ),
                ...
                records.append(
                    {
                        ...
                        "sweep_status": (
                            "evaluated" if len(group) > 0 else "no_candidate_rows"
                        ),
                    }
                )
