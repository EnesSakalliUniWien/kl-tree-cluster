...
GENERATED_BY = "benchmarks.diagnostics.calibration.spectral_transport_promotion_gate"
...
def evaluate_standard_dispatch_component(
    ...
    *,
    ...
    if missing:
        raise ValueError(
            f"dispatch pairwise rows missing required columns: {sorted(missing)!r}."
        )
    ...
    both_ok = (
        table["baseline_status"].astype(str).eq("ok")
        & table["candidate_status"].astype(str).eq("ok")
    )
...
def _paired_selected_family_rows(
    ...
    *,
    ...
    if missing:
        raise ValueError(
            f"selected-family rows missing required columns: {sorted(missing)!r}."
        )
...
def evaluate_selected_family_null_component(
    ...
    *,
    ...
    n_cases = int(paired["case_id"].nunique()) if not paired.empty else 0
    baseline_false = (
        paired["false_split_baseline"].map(_to_bool)
        if not paired.empty
        else pd.Series(dtype=bool)
    )
    candidate_false = (
        paired["false_split_candidate"].map(_to_bool)
        if not paired.empty
        else pd.Series(dtype=bool)
    )
