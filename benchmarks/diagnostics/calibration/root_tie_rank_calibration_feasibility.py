...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.root_tie_rank_calibration_feasibility"
)
...
def tail_precision_required_null_count(
    *,
    ...
    return int(
        math.ceil(
            (1.0 - float(target_alpha))
            / (float(target_alpha) * float(relative_se_target) ** 2)
        )
    )
...
def _annotate_conditioning_strata(mixed_rows: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in mixed_rows.iterrows():
        ...
        tie_fraction = _finite_float(
            row.get("root_tie_rank_median_fraction", math.nan)
        )
        edge_margin = _finite_float(
            row.get("root_edge_path_statistic_margin", math.nan)
        )
    ...
    *,
    ...
    for _, row in annotated.iterrows():
        ...
        if math.isfinite(observed_ratio) and not null_ratios.empty:
            ...
            empirical_p = (float(exceedance_count) + 1.0) / (
                float(null_ratios.shape[0]) + 1.0
            )
        else:
            exceedance_count = 0
        ...
        records.append(
            {
                ...
                "root_mixed_region_component": str(
                    row["root_mixed_region_component"]
                ),
                ...
                "root_bandwidth_reopen_count": _finite_float(
        ...
        records.append(
            {
                ...
                "stratum_feasibility_status": str(
                    group["calibration_feasibility_status"].iloc[0]
                ),
            }
        )
...
def summarize_root_tie_rank_calibration_feasibility(
    ...
    missing_resolution = int(
        strata["additional_null_count_for_alpha_resolution"].sum()
    )
