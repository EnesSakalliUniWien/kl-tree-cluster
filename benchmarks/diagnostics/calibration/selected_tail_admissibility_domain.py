...
def parse_run_specs(raw_specs: list[str]) -> tuple[SelectedTailRun, ...]:
    ...
    for raw_spec in raw_specs:
        if "=" not in raw_spec:
            raise ValueError(
                "Selected-tail run specs must use run_id=csv_path; "
                f"got {raw_spec!r}."
            )
...
def _validate_selected_tail_table(table: pd.DataFrame, *, path: Path) -> None:
    ...
    if missing:
        raise ValueError(
            f"Selected-tail table {path} is missing required columns: "
            f"{sorted(missing)!r}."
        )
...
def build_admissibility_domain_table(
    ...
    for run in runs:
        ...
        table["simulation_support_margin"] = (
            table["n_matching_simulations"]
            - table["required_min_matching_simulations"]
        )
        table["record_support_margin"] = (
