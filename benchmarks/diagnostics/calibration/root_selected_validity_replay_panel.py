...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.root_selected_validity_replay_panel"
)
...
def _root_replay_summary(
    ...
    *,
    ...
    selective_alpha = (
        float(np.nanmin(alpha_values))
        if alpha_values.size
        else float(configured_selective_alpha)
    )
    return {
        ...
        "root_replay_min_ari_to_observed": float(np.min(ari_values))
        if replay_count
        else math.nan,
...
def _root_validity_status(summary: dict[str, object]) -> tuple[str, str, str]:
    ...
    selective_would_block = bool(
        summary["root_selective_permutation_guard_would_block"]
    )
    ...
    if selective_blocked or selective_would_block or (
        has_selective and selective_p > selective_alpha
    ):
        return (
...
def build_root_selected_validity_replay_rows(
    *,
    ...
    replay_rows = (
        pd.DataFrame()
        if root_replay_rows is None
        else root_replay_rows.copy()
    )
    ...
    for _, tail in tail_rows.sort_values("target_case_id").iterrows():
        ...
        validity_status, validity_action, validity_interpretation = (
            _root_validity_status(replay_summary)
        )
        tail_status = _string_value(tail, "root_tail_inference_status")
        usability_status, method_action, usability_interpretation = (
            _selected_root_usability(
        ...
        records.append(
            {
                ...
                "root_validity_problem": (
                    "was_the_selected_root_bifurcation_stable_and_coherent"
                ),
                "root_tail_problem": (
                    "given_a_valid_selected_root_is_the_spectral_tail_supported"
                ),
                "selected_root_family_condition": (
        ...
        [
            {
                ...
                "median_root_replay_mean_ari_to_observed": float(
                    replay_mean.median()
                )
