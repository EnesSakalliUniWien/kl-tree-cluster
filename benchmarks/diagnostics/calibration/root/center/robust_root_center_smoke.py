...
GENERATED_BY = "benchmarks.diagnostics.calibration.root.center.robust_root_center_smoke"
DEFAULT_OUTPUT_DIR = Path("raw/assets/benchmark-results/robust_root_center_smoke_20260625")
...
def _halfspace_depth_scores(
    ...
    *,
    ...
    for direction_index in range(n_directions):
        ...
        depths[:, direction_index] = np.minimum(left_mass, right_mass) / n_observations
...
def simulate_branched_cloud(
    *,
    ...
    if imbalanced:
        sizes = np.maximum(
            8,
            np.round(n_per_cluster * np.linspace(0.25, 1.75, branch_count)),
        ).astype(int)
    else:
    ...
    if outlier_count:
        ...
        label_vector = np.concatenate([label_vector, np.full(outlier_count, -1)])
...
def branch_distance_cv(
    ...
    branch_means = np.vstack(
        [clean_matrix[clean_labels == label].mean(axis=0) for label in sorted(set(clean_labels))]
    )
...
def run_robust_root_center_smoke(
    ...
    for dimension in config.dimensions:
        for scenario_index, scenario in enumerate(_scenario_grid(config)):
            for replicate in range(config.replicates):
                seed = (
                    config.base_seed + 100_000 * int(dimension) + 1_000 * scenario_index + replicate
                )
                ...
                observed_tukey, observed_tukey_depth = approximate_tukey_observed_medoid(
                    matrix,
                    rng=np.random.default_rng(seed + 11),
                    n_directions=config.random_depth_directions,
                )
                candidate_tukey, candidate_tukey_depth = approximate_tukey_candidate_center(
                    matrix,
    ...
    summary["root_error_rank"] = summary.groupby(["dimension", "scenario"])["root_error_mean"].rank(
        method="dense"
    )
