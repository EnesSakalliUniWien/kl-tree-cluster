...
GENERATED_BY = "benchmarks.diagnostics.calibration.root.selected.root_selected_region_margins"
...
def _validate_linkage_inputs(
    ...
    if linkage_array.ndim != 2 or linkage_array.shape[1] != 4:
        raise ValueError(
            f"linkage_matrix must have shape (n_leaves - 1, 4); got {linkage_array.shape}."
        )
...
def replay_average_linkage_margins(
    ...
    *,
    ...
    if null_feature_covariance is not None and matrix is None:
        raise ValueError("null_feature_covariance requires Euclidean leaf_matrix geometry.")
    ...
    for step_index, row in enumerate(linkage_array):
        ...
        selected_tie_break_status = (
            "unique_minimum" if selected_tie_count == 1 else "tied_minimum_lexicographic_rank"
        )
        competitors = [(pair, score) for pair, score in candidate_items if pair != selected_key]
        if competitors:
        ...
        rows.append(
            {
                ...
                "nearest_competitor_left_members": _format_cluster_members(competitor_left_members),
...
def annotate_root_child_construction_roles(
    ...
    *,
    ...
    descendant_internal_ids: dict[int, set[int]] = {leaf_id: set() for leaf_id in range(n_leaves)}
...
def summarize_root_child_margin_geometry(
    ...
    *,
    ...
    defined = construction[construction["merge_margin_status"].eq("defined")].copy()
    if construction.empty:
        ...
        tied_minimum_count = int(defined["selected_pair_tied_for_minimum"].astype(bool).sum())
        margin_status = "defined"
        smooth_constraints = defined[
            defined["selected_region_geometry_status"].eq("smooth_first_order_geometry_defined")
        ]
        ...
        if smooth_count:
            signed_distances = smooth_constraints["merge_first_order_signed_distance"].to_numpy(
                dtype=float
            )
            ...
            if null_whitened_count:
                ...
                median_null_whitened_distance = float(np.quantile(null_whitened_distances, 0.5))
            else:
                min_null_whitened_distance = np.nan
                median_null_whitened_distance = np.nan
        else:
        ...
        if discrete_tie_cell_count:
            ...
            selected_region_law_status = "null_whitened_first_order_signed_distance_defined"
    ...
    for side in ("left", "right"):
        ...
        if side_defined.empty:
            ...
            side_rows[f"{side}_root_child_construction_merge_count"] = int(side_defined.shape[0])
    ...
    summary = {
        ...
        "root_child_min_null_whitened_first_order_signed_distance": (min_null_whitened_distance),
...
def fixed_projection_edge_conditioned_sibling_tail(
    *,
    ...
    if sibling_df < 0:
        raise ValueError(f"sibling_degrees_of_freedom must be non-negative; got {sibling_df!r}.")
    if edge_df <= 0:
    ...
    if sibling_df == 0:
        ...
        return {
            "edge_conditioned_sibling_law_status": ("evaluated_zero_sibling_projection_dimension"),
    ...
    if sibling_df == edge_df:
        ...
        if lower < threshold:
            integral_part, _error = integrate.quad(
                lambda x: (
                    stats.chi2.pdf(x, df=sibling_df) * stats.chi2.sf(threshold - x, df=extra_df)
                ),
                lower,
                threshold,
                epsabs=1e-12,
...
def root_edge_sibling_wald_relationship(
    ...
    *,
    ...
    if projection.ndim != 2:
        raise ValueError(f"Parent projection for {parent!r} must be 2-D; got {projection.shape}.")
    ...
    return {
        ...
        "root_sibling_projection_dimension_for_relationship": int(sibling_projection_dimension),
        "root_edge_sibling_projection_prefix_status": "sibling_uses_parent_projection_prefix",
        "root_sibling_recomputed_statistic_from_parent_projection": (sibling_projected_statistic),
        "root_edge_equivalent_parent_projection_statistic": (edge_equivalent_parent_statistic),
...
def _root_spectral_summary(
    *,
    ...
    if projected_eigenvalues.ndim != 1:
        raise ValueError(
            f"Root projected eigenvalues must be 1-D; got {projected_eigenvalues.shape}."
        )
    ...
    if full_eigenvalues.ndim != 1:
        raise ValueError(f"Root full eigenvalues must be 1-D; got {full_eigenvalues.shape}.")
    ...
    return {
        ...
        "root_raw_mp_signal_count": int(spectral_context.raw_mp_signal_counts_by_node[node_id]),
...
def collect_observed_root_selected_region_row(
    ...
    *,
    ...
    left_edge_statistic = float(edge_df.loc[left_child, "Child_Parent_Divergence_Test_Statistic"])
    right_edge_statistic = float(edge_df.loc[right_child, "Child_Parent_Divergence_Test_Statistic"])
    left_edge_degrees_of_freedom = float(edge_df.loc[left_child, "Child_Parent_Divergence_df"])
    right_edge_degrees_of_freedom = float(edge_df.loc[right_child, "Child_Parent_Divergence_df"])
    ...
    sibling_recomputed_statistic = float(
        edge_sibling_relationship["root_sibling_recomputed_statistic_from_parent_projection"]
    )
    ...
    row = {
        ...
        "root_edge_opening_boundary_status": left_edge_geometry["edge_opening_boundary_status"],
        ...
        "root_left_edge_chi_square_threshold": left_edge_geometry["edge_chi_square_threshold"],
        "root_right_edge_chi_square_threshold": right_edge_geometry["edge_chi_square_threshold"],
        "root_left_edge_statistic_margin": left_edge_geometry["edge_statistic_margin"],
        "root_right_edge_statistic_margin": right_edge_geometry["edge_statistic_margin"],
        ...
        **edge_sibling_relationship,
        ...
        "root_sibling_edge_conditioned_blocks_at_sibling_alpha": (
            edge_conditioned_sibling_tail["edge_conditioned_sibling_blocks_at_sibling_alpha"]
        ),
...
def summarize_root_selected_region_relationships(root_table: pd.DataFrame) -> pd.DataFrame:
    ...
    missing_covariates = [
        covariate for covariate in RELATIONSHIP_COVARIATES if covariate not in root_table.columns
    ]
...
def _make_output_dir(output_dir: Path | None) -> Path:
    if output_dir is not None:
        ...
        resolved = Path("benchmarks") / "results" / f"root_selected_region_margins_{stamp}"
