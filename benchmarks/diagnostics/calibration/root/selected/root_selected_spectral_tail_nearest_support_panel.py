...
GENERATED_BY = "benchmarks.diagnostics.calibration.root.selected.root_selected_spectral_tail_nearest_support_panel"
...
def _nearest_support(
    *,
    ...
    for _, candidate in support.iterrows():
        ...
        if (
            best_row is None
            or components["nearest_support_distance"] < best_components["nearest_support_distance"]
        ):
            best_row = candidate
...
def summarize_root_selected_spectral_tail_nearest_support_rows(
    ...
    fail_closed = int(
        rows["production_inference_status"].astype(str).eq("fail_closed_nearest_support_only").sum()
    )
