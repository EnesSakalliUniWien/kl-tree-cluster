...
def prepare_selected_edge_sibling_equation_records(records: pd.DataFrame) -> pd.DataFrame:
    ...
    table["edge_action_bin"] = [
        _bin_value(float(value), EDGE_ACTION_BINS, EDGE_ACTION_BIN_LABELS)
        for value in edge_action
    ]
        ...
        summaries.append(
            {
                ...
                "p_value_q50": float(p_values.quantile(0.50)) if p_values.notna().any() else math.nan,
                "p_value_q10": float(p_values.quantile(0.10)) if p_values.notna().any() else math.nan,
                "study_role": STUDY_ROLE,
            }
        )
