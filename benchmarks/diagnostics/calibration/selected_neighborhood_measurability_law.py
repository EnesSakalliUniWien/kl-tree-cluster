...
GENERATED_BY = (
    ...
    "selected_neighborhood_measurability_law"
)
...
def compute_child_interpolated_null_prior(
    *,
    ...
    """
    ancestor_p = [validate_probability(value, label="ancestor_p_value") for value in ancestor_p_values]
...
def _interpolation_contract_status(
    ...
    *,
    ...
    topology_support_count = _finite_int(
        row.get("topology_neighborhood_support_count", 0.0)
    )
    support_anchor_count = _finite_int(row.get("support_anchor_count", 0.0))
    support_count = (
        topology_support_count if topology_support_count > 0 else support_anchor_count
    )
    topology_signal_count = _finite_int(
        row.get("topology_neighborhood_signal_count", 0.0)
    )
    ...
    excluded_count = (
        topology_excluded_count
        if topology_excluded_count > 0
        else anchor_excluded_count
    )
...
def _topology_coherence_status(
    ...
    *,
    ...
    if not math.isfinite(balance_product):
        structural_incoming = _finite_float(
            row.get("structural_incoming_branch_balance", math.nan)
        )
        structural_outgoing = _finite_float(
...
def evaluate_measurability_decision(
    ...
    *,
    ...
    interpolation_measurable, interpolation_status, pair_prior, *_ = (
        _interpolation_contract_status(row, min_support=int(min_interpolation_support))
    )
    ...
    if direct_measurable:
        direct_open = sibling_open or (
            math.isfinite(p_value) and p_value <= float(direct_alpha)
        )
        if direct_open:
    ...
    *,
    ...
    for _, row in rows.iterrows():
        ...
        record = {
            ...
            "n_descendant_leaves": _finite_float(
                row.get("n_descendant_leaves", math.nan)
            ),
            "structural_n_parent_context": _finite_float(
                row.get("structural_n_parent_context", math.nan)
            ),
            "structural_n_node": _finite_float(
                row.get("structural_n_node", math.nan)
            ),
            "structural_n_incoming_sibling": _finite_float(
                row.get("structural_n_incoming_sibling", math.nan)
            ),
            "structural_n_left": _finite_float(
                row.get("structural_n_left", math.nan)
            ),
            "structural_n_right": _finite_float(
                row.get("structural_n_right", math.nan)
            ),
            ...
            "interpolation_support_weight": _finite_float(
                row.get("support_weight", math.nan)
            ),
            ...
            "incoming_branch_balance": _finite_float(
                row.get("incoming_branch_balance", math.nan)
            ),
            ...
            "spectral_mp_pair_supported": _bool_value(
                row.get("spectral_mp_pair_supported", False)
            ),
            ...
            "spectral_flow_affinity": _finite_float(
                row.get("spectral_flow_affinity", math.nan)
            ),
...
def _add_selected_tree_structural_topology(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped = enriched.groupby(group_columns, dropna=False, sort=False) if group_columns else [((), enriched)]
    ...
    for _keys, group in grouped:
        ...
        parent_by_node = {
            str(row.node_id): (
                "" if pd.isna(row.parent_id) else str(row.parent_id)
            )
            for row in group.itertuples()
        }
