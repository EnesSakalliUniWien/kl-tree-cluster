...
STUDY_ROLE = (
    "diagnostic_root_tie_rank_spectral_action_dominance_panel_not_calibration"
)
GENERATED_BY = (
    "benchmarks.diagnostics.calibration."
    "root_tie_rank_spectral_action_dominance_panel"
)
...
def _dominance_record(target: pd.Series, generated: pd.Series) -> dict[str, object]:
    ...
    generated_tie = _finite_float(
        generated.get("root_tie_rank_median_fraction", math.nan)
    )
    target_action = _safe_log1p(target.get("root_sibling_selected_ratio", math.nan))
    generated_action = _safe_log1p(
        generated.get("root_sibling_selected_ratio", math.nan)
    )
    target_edge = _safe_log1p(target.get("root_edge_path_statistic_margin", math.nan))
    generated_edge = _safe_log1p(
    ...
    full_dominates = bool(
        tie_dominates
        and selected_ratio_dominates
        and edge_dominates
        and spectral_dominates
    )
        ...
        records.append(
            {
                ...
                "median_dominance_deficit_score": float(
                    group["dominance_deficit_score"].median()
                ),
                "min_dominance_deficit_score": float(
                    group["dominance_deficit_score"].min()
                ),
                "median_spectral_log_deficit": float(
                    group["spectral_log_deficit"].median()
                ),
