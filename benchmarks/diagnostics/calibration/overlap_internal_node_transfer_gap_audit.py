...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.overlap_internal_node_transfer_gap_audit"
)
...
def summarize_transfer_gap_rows(
    ...
    bayes_blocked_count = int(
        (truth & status.eq("selected_family_bayes_evidence_blocked")).sum()
    )
        ...
        [
            {
                ...
                "truth_recovered_by_default_count": int(
                    (
                        truth
                        & status.eq("recovered_by_default_internal_node_likelihood")
                    ).sum()
                ),
                "truth_context_negative_soft_supported_count": (
...
def run_overlap_internal_node_transfer_gap_audit(
    ...
    transfer_rows = (
        pd.read_csv(config.transfer_rows_path)
        if config.transfer_rows_path is not None
        else None
    )
