...
STUDY_ROLE = (
    "diagnostic_root_tie_rank_measured_coupling_residual_panel_not_calibration"
)
GENERATED_BY = (
    "benchmarks.diagnostics.calibration."
    "root_tie_rank_measured_coupling_residual_panel"
)
...
def _json_default(value: object) -> object:
    if isinstance(value, RootTieRankMeasuredCouplingResidualConfig):
        return asdict(value)
...
def _best_row_for_target(group: pd.DataFrame) -> pd.Series:
    ranked = group.copy()
    ranked["_measured_dominates_rank"] = ranked[
        "measured_neighborhood_coupling_dominates"
    ].map(_bool_value)
    ranked["_measured_ratio_rank"] = ranked[
        "measured_neighborhood_coupling_ratio"
    ].map(_finite_or_negative_inf)
    ranked["_bottleneck_ratio_rank"] = ranked["bottleneck_coupling_ratio"].map(
        _finite_or_negative_inf
    )
    ranked["_measured_deficit_rank"] = ranked[
        "measured_neighborhood_coupling_deficit"
    ].map(_finite_or_positive_inf)
    ranked["_score_rank"] = ranked["coupling_deficit_score"].map(
...
def _residual_record(row: pd.Series, *, partial_ratio_floor: float) -> dict[str, object]:
    ...
    measured_reached = _bool_value(
        row.get("measured_neighborhood_coupling_dominates", False)
    )
...
def summarize_measured_coupling_residual_rows(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, group in rows.groupby(group_columns, dropna=False, sort=True):
        ...
        calibration_count = int(
            group["proposal_support_role"]
            .astype(str)
            .eq("selected_null_candidate_support")
            .sum()
        )
