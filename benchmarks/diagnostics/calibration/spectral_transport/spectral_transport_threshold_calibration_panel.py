...
GENERATED_BY = "benchmarks.diagnostics.calibration.spectral_transport.spectral_transport_threshold_calibration_panel"
...
def _spectral_annotation_counts(annotations: object) -> dict[str, float]:
    ...
    return {
        "spectral_transport_blocked_count": _bool_sum("Spectral_Transport_Pass_Through_Blocked"),
...
def _run_one_variant(
    *,
    ...
    counts = _spectral_annotation_counts(result.extra.get("annotations") if result.extra else None)
    return {
        ...
        "false_split": bool(result.status == "ok" and found_clusters > int(true_clusters)),
...
def build_threshold_pairwise_rows(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    out["false_split_reduced"] = out["baseline_false_split"].astype(bool) & ~out[
        "candidate_false_split"
    ].astype(bool)
...
def summarize_thresholds(
    ...
    *,
    ...
    for threshold, group in pairwise.groupby(
        ...
        min_signal_delta = float(signal_delta.min()) if not signal_delta.empty else np.nan
        mean_signal_delta = float(signal_delta.mean()) if not signal_delta.empty else np.nan
        signal_regression_count = int((signal_delta < -float(max_allowed_signal_ari_drop)).sum())
        null_pass = bool(false_split_reduction >= int(min_null_false_split_reduction))
        signal_pass = bool(
            not signal_delta.empty and min_signal_delta >= -float(max_allowed_signal_ari_drop)
        )
