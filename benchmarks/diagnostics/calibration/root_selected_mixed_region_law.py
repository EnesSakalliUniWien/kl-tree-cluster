...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.root_selected_mixed_region_law"
)
...
def _aggregate_root_frontier(
    ...
    if "candidate_scope" in frontier.columns:
        ...
        if "parent_id" in frontier.columns:
            root_mask = root_mask | (
                scope.eq("direct_measurable")
                & frontier["parent_id"].fillna("").astype(str).eq("")
            )
...
def _region_component(
    *,
    ...
    has_smooth = (
        law_status == "smooth_first_order_signed_distance_defined"
        or smooth_count > 0
    )
...
def build_root_selected_mixed_region_law_rows(
    *,
    ...
    for _, root in root_summary.sort_values("case_id").iterrows():
        ...
        rank_mean_fraction = _finite_float(
            tie.get("root_tie_rank_mean_fraction", math.nan)
        )
        rank_median_fraction = _finite_float(
            tie.get("root_tie_rank_median_fraction", math.nan)
        )
        representative_rank_fraction = (
            rank_median_fraction
            if math.isfinite(rank_median_fraction)
            else rank_mean_fraction
        )
        ...
        rank_spectral_product = (
            ...
            if math.isfinite(representative_rank_fraction)
            and math.isfinite(spectral_ratio)
            else math.nan
        )
        ...
        hybrid_support_count = int(
            frontier.get("root_hybrid_strict_support_count", 0)
        )
        ...
        records.append(
            {
                ...
                "root_child_balance": _finite_float(
                    root.get("root_child_balance", math.nan)
                ),
                "root_child_construction_merge_count": int(
                    _finite_float(
                        root.get("root_child_construction_merge_count", 0)
                    )
                ),
                ...
                "root_child_tied_minimum_merge_count": int(
                    _finite_float(
                        root.get("root_child_tied_minimum_merge_count", 0)
                    )
                ),
                ...
                "root_mp_upper_bound": _finite_float(
                    root.get("root_mp_upper_bound", math.nan)
                ),
                ...
                "root_frontier_data_roles": str(
                    frontier.get("root_frontier_data_roles", "")
                ),
        ...
        records.append(
            {
                ...
                "median_tie_rank_fraction": _finite_median(
                    group["root_tie_rank_median_fraction"]
                ),
    ...
    run_root_selected_mixed_region_law(
        RootSelectedMixedRegionLawConfig(
            ...
            topology_frontier_rows_path=(
                None
                if bool(args.no_topology_frontier)
                else args.topology_frontier_rows_path
            ),
