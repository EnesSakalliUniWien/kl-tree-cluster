...
from benchmarks.diagnostics.calibration.selected_neighborhood_measurability_law import (
...
STUDY_ROLE = (
    "diagnostic_selected_neighborhood_pvalue_interpolation_comparison_not_calibration"
)
GENERATED_BY = (
    "benchmarks.diagnostics.calibration."
    "selected_neighborhood_pvalue_interpolation_comparison"
)
...
def _support_and_signal_masks(
    ...
    *,
    ...
    has_explicit_roles = bool(support_role.str.len().gt(0).any() or signal_role.str.len().gt(0).any())
    ...
    if has_explicit_roles:
        support_mask = (
            ...
            & ~selected_nonnull
            & (
                support_role.isin(ELIGIBLE_SUPPORT_ROLES)
                | guard_role.eq("null_like")
            )
        )
...
def _build_group_interpolation_context(
    ...
    *,
    ...
    p_values = pd.to_numeric(group.get("sibling_p_value"), errors="coerce").to_numpy(
        dtype=float
    )
...
def compute_holdout_interpolated_p_like(
    *,
    ...
    return HoldoutInterpolatedPValue(
        ...
        nearest_support_distance=float(min(support_distances)),
        nearest_signal_distance=(
            float(min(signal_distances)) if signal_distances else math.inf
        ),
        tau_t=float(tau_t),
        tau_s=float(tau_s),
        h_k=float(h_k),
        ...
        records.append(
            {
                ...
                "selected_nonnull_excluded_count": (
                    interpolation.selected_nonnull_excluded_count
                ),
        ...
        records.append(
            {
                ...
                "interpolated_significant_count": int(
                    group["interpolated_significant"].sum()
                ),
                "interpolation_lower_count": int(
                    group["comparison_class"]
                    .eq("interpolation_lower_than_direct")
                    .sum()
                ),
                "interpolation_higher_count": int(
                    group["comparison_class"]
                    .eq("interpolation_higher_than_direct")
                    .sum()
                ),
                ...
                "median_negative_log10_delta": _safe_median(
                    group["negative_log10_delta"]
                ),
...
def summarize_pvalue_interpolation_cases(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, group in rows.groupby(["case_id", "data_role", "method_id"], dropna=False):
        ...
        dominant_behavior = (
            str(behavior_counts.index[0]) if not behavior_counts.empty else ""
        )
            ...
            records.append(
                {
                    ...
                    "best_case_significant_fraction": (
                        float(count / total) if total else math.nan
                    ),
...
def run_pvalue_interpolation_comparison(
    *,
    ...
    case_summary_out = (
        output_dir / "selected_neighborhood_pvalue_interpolation_case_summary.csv"
    )
