...
GENERATED_BY = (
    ...
    "overlap_selected_pass_through_fixture_miner"
)
...
def _optional_context_by_key(
    ...
    missing = sorted(
        set(SELECTED_NEIGHBORHOOD_OPTIONAL_COLUMNS[:6])
        - set(selected_neighborhood_rows.columns)
    )
    if missing:
...
def _truth_context_by_key(
    ...
    if truth_context_rows is None or truth_context_rows.empty:
        return {}
    missing = sorted(
        set(TRUTH_CONTEXT_COLUMNS[:5])
        - set(truth_context_rows.columns)
    )
    if missing:
...
def build_structural_topology_context_rows(
    ...
    for group_key, group in rows.groupby(
        ...
        for node_id in node_ids:
            ...
            incoming_siblings = [
                sibling
                for sibling in children_by_node.get(parent_id, [])
                if sibling != node_id
            ]
            ...
            feature_count = int(math.isfinite(incoming_balance)) + int(
                math.isfinite(outgoing_balance)
    ...
    for group_key, group in node_rows.groupby(group_columns, sort=False):
        ...
        def split_metrics(split_node_id: str) -> dict[str, object] | None:
            ...
            return {
                ...
                "child_mean_purity": float(
                    np.nanmean(
                        [_purity_from_labels(left_truth), _purity_from_labels(right_truth)]
                    )
                ),
                "child_majority_distinct": bool(
...
def build_overlap_selected_pass_through_node_rows(
    ...
    for _, row in selected.sort_values(
        ...
        topology_support_role = (
            "" if node is None else str(node.get("topology_support_role", ""))
        )
        topology_signal_role = (
            "" if node is None else str(node.get("topology_signal_role", ""))
        )
        ...
        truth_geometry_role = (
            "" if truth is None else str(truth.get("truth_geometry_role", ""))
        )
        truth_context_status = (
        ...
        structural_feature_count = (
            ...
            else int(structural["structural_topology_feature_count"])
        )
        structural_balance_product = (
            math.nan
            if structural is None
        ...
        records.append(
            {
                ...
                "data_seed": (
                    math.nan if truth is None else _finite_float(truth["data_seed"])
                ),
                "truth_geometry_role": truth_geometry_role,
                ...
                "truth_node_sample_count": (
                    0
                    if truth is None
                    else _finite_int(truth["truth_node_sample_count"])
                ),
                "truth_node_cluster_count": (
                    0
                    if truth is None
                    else _finite_int(truth["truth_node_cluster_count"])
                ),
                "truth_node_majority_fraction": (
                    math.nan
                    if truth is None
                ...
                "truth_downstream_child_majority_distinct": (
                    ...
                    if truth is None
                        ...
                        truth["truth_downstream_child_majority_distinct"]
                    )
                ),
                "topology_feature_status": _topology_feature_status(
                    finite_topology_count
                ),
                "left_depth": _finite_float(row["left_depth"]),
                "left_n_descendant_leaves": _finite_float(
                    row["left_n_descendant_leaves"]
                ),
                "left_sibling_p_value": _finite_float(row["left_sibling_p_value"]),
                "left_child_parent_edge_open": _bool_value(
                    row["left_child_parent_edge_open"]
                ),
                ...
                "left_neighborhood_evidence_family": str(
                    row["left_neighborhood_evidence_family"]
                ),
                ...
                "structural_incoming_branch_balance": (
                    ...
                    if structural is None
                        ...
                        structural["structural_incoming_branch_balance"]
                    )
                ),
                "structural_outgoing_balance": (
                    math.nan
                ...
                "distance_to_downstream_accepted_split": (
                    ...
                    if traversal is None
                        ...
                        traversal["distance_to_downstream_accepted_split"]
                    )
                ),
                "network_component_node_count": (
                    math.nan
...
def build_overlap_selected_pass_through_case_rows(
    ...
    for keys, group in node_rows.groupby(
        ...
        truth_labeled_signal_count = int(
            group["truth_geometry_role"].astype(str).isin(
                {
                    "truth_recovery_pass_through_positive",
                    "partial_truth_recovery_pass_through_candidate",
                    "barycentric_mixture_pass_through_candidate",
        ...
        records.append(
            {
                ...
                "truth_recovery_count": int(
                    group["fixture_role"]
                    .eq("truth_recovery_pass_through_positive")
                    .sum()
                ),
                "partial_truth_recovery_count": int(
                    group["fixture_role"]
                    .eq("partial_truth_recovery_pass_through_candidate")
                    .sum()
                ),
...
def summarize_overlap_selected_pass_through_support(
    ...
    *,
            ...
            [
                {
                    ...
                    "completed_fixture_support_status": (
                        "no_selected_pass_through_fixture_rows"
                    ),
                    "completed_balance_product_signal_min": math.nan,
                    "completed_balance_product_signal_median": math.nan,
                    "completed_balance_product_signal_max": math.nan,
    ...
    finite_selected_null_topology = int(
        selected_null["finite_topology_feature_count"].gt(0).sum()
    )
    finite_signal_structural = int(
        signal["structural_topology_feature_count"].gt(0).sum()
    )
    finite_selected_null_structural = int(
        selected_null["structural_topology_feature_count"].gt(0).sum()
    )
    finite_signal_completed = int(
        signal["completed_topology_feature_count"].gt(0).sum()
    )
    ...
    truth_labeled_signal_count = int(
        signal["truth_geometry_role"].astype(str).isin(
            {
                "truth_recovery_pass_through_positive",
                "partial_truth_recovery_pass_through_candidate",
                "barycentric_mixture_pass_through_candidate",
...
def run_overlap_selected_pass_through_fixture_miner(
    ...
    if config.traversal_rows_path is not None or config.gene_assignments_path is not None:
        ...
        truth_context_rows = build_selected_pass_through_truth_context_rows(
            ...
            min_partial_recovery_ari=float(
                config.truth_partial_recovery_ari_floor
            ),
    ...
    manifest = {
        ...
        "inputs": {
            ...
            "gene_assignments": (
                None
                if config.gene_assignments_path is None
        ...
        "truth_context": {
            ...
            "truth_partial_recovery_ari_floor": float(
                config.truth_partial_recovery_ari_floor
            ),
        ...
        "support_thresholds": {
            ...
            "min_selected_null_control_count": int(
                config.min_selected_null_control_count
            ),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    outputs = run_overlap_selected_pass_through_fixture_miner(
        OverlapSelectedPassThroughFixtureMinerConfig(
            ...
            truth_partial_recovery_ari_floor=float(
                args.truth_partial_recovery_ari_floor
            ),
