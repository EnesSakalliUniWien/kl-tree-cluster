...
from benchmarks.diagnostics.calibration.overlap.overlap_conditional_bayesian_traversal_law import (
...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.overlap.overlap_internal_node_bayesian_likelihood_probe"
)
...
class OverlapInternalNodeBayesianLikelihoodProbeConfig:
    ...
    parameters: OverlapInternalNodeLikelihoodParameters = OverlapInternalNodeLikelihoodParameters()
...
def build_internal_node_likelihood_rows(
    ...
    *,
    ...
    residual = policy_rows[policy_rows["diagnostic_traversal_action"].astype(str).eq(str(action))][
        keys + ["guard_truth_role", "truth_geometry_mode"]
    ].copy()
    ...
    status.loc[
        p_pass & context_pass & subspace_pass & (~size_pass | ~edge_pass | ~fragment_pass)
        ...
        records.append(
            {
                ...
                "context_blocked_count": int(status.eq("row_overlap_context_blocked").sum()),
                "subspace_blocked_count": int(status.eq("row_overlap_subspace_blocked").sum()),
...
def summarize_internal_node_likelihood_families(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    for (case_id, data_role, replicate), group in rows.groupby(group_cols, sort=True):
        ...
        recovery_candidates = candidate_rows["guard_truth_role"].astype(str).eq("truth_recovery")
        nonrecovery_candidates = candidate_rows["guard_truth_role"].astype(str).ne("truth_recovery")
        ...
        records.append(
            {
                ...
                "nonrecovery_or_null_candidate_row_count": int(nonrecovery_candidates.sum()),
