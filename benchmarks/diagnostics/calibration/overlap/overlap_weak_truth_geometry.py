...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap.overlap_weak_truth_geometry"
...
def classify_truth_geometry(
    *,
    ...
    if float(max_child_truth_purity) >= float(high_child_purity) and float(
        min_child_truth_purity
    ) < float(low_child_purity):
        return "one_sided_pure_fragment"
    if float(max_child_truth_purity) >= float(balanced_child_purity) and float(
        child_truth_purity_gap
    ) >= float(purity_gap):
        ...
        records.append(
            {
                ...
                "truth_aligned_row_count": int(
                    modes.isin({"balanced_truth_recovery", "partial_truth_recovery"}).sum()
                ),
                ...
                "wrong_granularity_count": int(modes.eq("balanced_but_wrong_granularity").sum()),
                "diffuse_mismatch_count": int(modes.eq("diffuse_truth_mismatch").sum()),
                "max_truth_split_ari": float(group["truth_split_ari"].max()),
                "median_truth_split_ari": float(group["truth_split_ari"].median()),
                "max_child_truth_purity": float(group["max_child_truth_purity"].max()),
                "median_min_child_truth_purity": float(group["min_child_truth_purity"].median()),
                "median_child_truth_purity_gap": float(group["child_truth_purity_gap"].median()),
                "max_homogeneity_gain_min": float(group["homogeneity_gain_min"].max()),
            }
        )
...
def summarize_weak_truth_geometry(
    ...
    family_key = (
        family_geometry["case_id"].astype(str) + "::" + family_geometry["replicate"].astype(str)
    )
    records: list[dict[str, object]] = []
        ...
        records.append(
            {
                ...
                "median_truth_split_ari": float(group["truth_split_ari"].median()),
                "median_parent_truth_purity": float(group["parent_truth_purity"].median()),
                "median_min_child_truth_purity": float(group["min_child_truth_purity"].median()),
                "median_max_child_truth_purity": float(group["max_child_truth_purity"].median()),
                "median_child_truth_purity_gap": float(group["child_truth_purity_gap"].median()),
