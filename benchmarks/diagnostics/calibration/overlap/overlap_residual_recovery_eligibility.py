...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap.overlap_residual_recovery_eligibility"
...
def _thresholds(
    ...
    *,
    ...
    return {
        ...
        "nonrecovery_structural_threshold": _next_after_max(nonrecovery_rows[structural_metric]),
        "full_negative_structural_threshold": _next_after_max(all_negative_rows[structural_metric]),
    }
...
def assign_residual_recovery_eligibility(
    ...
    *,
    ...
    statuses.loc[null_pass & full_negative_pass] = "residual_strict_structural_recovery_candidate"
    return pd.DataFrame(
        {
            ...
            "residual_family_truth_role": family_rows["residual_family_truth_role"].astype(str),
            ...
            "nonrecovery_structural_threshold": thresholds["nonrecovery_structural_threshold"],
            "nonrecovery_structural_pass": nonrecovery_pass.astype(bool),
            "full_negative_structural_threshold": thresholds["full_negative_structural_threshold"],
        ...
        records.append(
            {
                ...
                "null_like_family_count": int(roles.eq("residual_null_like_family").sum()),
                "truth_recovery_family_count": int(
                    roles.eq("residual_truth_recovery_family").sum()
                ),
                "nonrecovery_family_count": int(roles.eq("residual_nonrecovery_family").sum()),
...
def summarize_residual_recovery_thresholds(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    truth = rows["residual_family_truth_role"].astype(str).eq("residual_truth_recovery_family")
    records: list[dict[str, object]] = []
    specs = (
        (
            "null_evidence_threshold",
    ...
    for threshold_name, metric_column, pass_column, negative_role in specs:
        if negative_role == "residual_null_or_nonrecovery_family":
            negative = (
                rows["residual_family_truth_role"]
                .astype(str)
                .isin({"residual_null_like_family", "residual_nonrecovery_family"})
            )
        else:
