...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.overlap.overlap_bayesian_neighborhood_component_audit"
)
    ...
    output = pd.DataFrame(
        {
            ...
            "conditional_bayesian_status": rows["conditional_bayesian_status"].astype(str),
...
def summarize_bayesian_neighborhood_components(
    ...
    for role, group in component_rows.groupby("residual_family_truth_role", sort=True):
        ...
        common_limiting = str(limiting.value_counts().idxmax()) if not limiting.empty else "none"
        records.append(
            {
                "schema_version": SCHEMA_VERSION,
                "study_role": STUDY_ROLE,
                ...
                "median_strong_neighborhood_gap": float(group["strong_neighborhood_gap"].median()),
                ...
                "p_value_evidence_without_strong_neighborhood_count": int(
                    status.eq("component_audit_p_value_evidence_without_strong_neighborhood").sum()
                ),
                ...
                "diagnostic_status": ("diagnostic_only_component_audit_not_calibration"),
            }
        )
...
def run_overlap_bayesian_neighborhood_component_audit(
    ...
    component_rows = build_bayesian_neighborhood_component_rows(
        ...
        p_extreme_log_bayes_factor_threshold=(config.p_extreme_log_bayes_factor_threshold),
    )
    ...
    manifest = {
        ...
        "p_extreme_log_bayes_factor_threshold": float(config.p_extreme_log_bayes_factor_threshold),
        "outputs": {
