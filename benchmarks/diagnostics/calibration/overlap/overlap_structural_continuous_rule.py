...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap.overlap_structural_continuous_rule"
...
def continuous_homogeneity_threshold(
    ...
    parent_size = pd.to_numeric(rows["n_parent"], errors="coerce").fillna(0.0).clip(lower=1.0)
    balance = (
...
def apply_continuous_rule(
    ...
    *,
    ...
    signal_truth_aligned_total_by_case_rep = (
        (accepted_split & signal_mask & truth_aligned).groupby(case_rep).sum()
    )
    ...
    signal_truth_aligned_accept_by_case_rep = (
        (accept & signal_mask & truth_aligned).groupby(case_rep).sum()
    )
    ...
    summary = {
        ...
        "signal_truth_aligned_retention": (float(retention) if not np.isnan(retention) else np.nan),
        "signal_truth_aligned_case_replicates_total": (signal_truth_aligned_case_replicates_total),
        "signal_truth_aligned_case_replicates_retained": (
            signal_truth_aligned_case_replicates_retained
        ),
