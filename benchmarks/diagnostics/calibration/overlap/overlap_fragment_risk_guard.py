...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap.overlap_fragment_risk_guard"
...
def build_fragment_guard_rows(
    ...
    *,
    ...
    balanced_score = min_pairwise + min_gain + edge_balance + size_balance + subspace - pairwise_gap
    ...
    for metric, direction in guard_specs:
        ...
        for threshold in thresholds:
            if direction == "greater_equal":
                ...
                raise ValueError(f"Unknown guard direction: {direction!r}.")
            counts = {role: int((blocked & roles.eq(role)).sum()) for role in totals}
            ...
            null_block_rate = (
                counts["null_like"] / totals["null_like"] if totals["null_like"] else math.nan
            )
            diffuse_block_rate = (
