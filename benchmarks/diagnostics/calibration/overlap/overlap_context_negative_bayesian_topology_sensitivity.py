...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap.overlap_context_negative_bayesian_topology_sensitivity"
...
class OverlapContextNegativeBayesianTopologySensitivityConfig:
    ...
    def sensitivity_rows_path(self) -> Path:
        return self.output_dir / "overlap_context_negative_bayesian_topology_sensitivity_rows.csv"

    @property
    def summary_path(self) -> Path:
        return (
            self.output_dir / "overlap_context_negative_bayesian_topology_sensitivity_summary.csv"
        )
        ...
        [
            {
                "schema_version": SCHEMA_VERSION,
                "study_role": STUDY_ROLE,
                "context_penalty_weight_count": int(rows["context_penalty_weight"].nunique()),
                "profile_count": int(rows["profile"].nunique()),
                "separating_profile_count": int(separates.sum()),
                "top_rank_profile_count": int(top_rank.sum()),
                "single_component_separating_count": int((single_component & separates).sum()),
                "topology_only_separates_count": int(topology_only.sum()),
                "outgoing_topology_only_separates_count": int(outgoing_topology_only.sum()),
                "selected_context_only_separates_count": int(selected_context_only.sum()),
                ...
                "minimum_outgoing_topology_only_margin": float(outgoing_margins.min())
...
def run_overlap_context_negative_bayesian_topology_sensitivity(
    ...
    manifest = {
        ...
        "component_profiles": {
            profile: list(components) for profile, components in DEFAULT_COMPONENT_PROFILES
        },
        "outputs": {
