...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap.overlap_structural_context_thresholds"
...
def _evaluate_context_threshold(
    ...
    *,
    ...
    structural_accept = (
        accepted_split
        & pd.to_numeric(group["sibling_p_value"], errors="coerce").le(float(sibling_p_threshold))
    ...
    signal_truth_aligned_accept_count = int((structural_accept & signal_mask & truth_aligned).sum())
    signal_truth_misaligned_accept_count = int(
    ...
    return {
        ...
        "median_parent_size": float(pd.to_numeric(group["n_parent"], errors="coerce").median()),
        ...
        records.append(
            {
                ...
                "context_axis": context_axis,
                "context_bin": context_bin,
                "best_homogeneity_gain_threshold": float(best["homogeneity_gain_threshold"]),
                "best_subspace_consensus_threshold": float(best["subspace_consensus_threshold"]),
                ...
                "signal_truth_aligned_accept_count": int(best["signal_truth_aligned_accept_count"]),
                "signal_truth_aligned_retention": float(best["signal_truth_aligned_retention"])
