...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap.overlap_structural_decision_zones"
...
def _truth_roles(
    ...
    *,
    ...
    roles.loc[signal & truth_ari.ge(float(truth_alignment_threshold))] = "signal_truth_aligned"
...
def continuous_context_margins(
    ...
    log_p_margin.loc[valid_p] = np.log(float(parameters.sibling_p_threshold)) - np.log(
        clipped_p.loc[valid_p].astype(float)
    )
...
def assign_structural_decision_zones(
    ...
    *,
    ...
    zones.loc[accepted_split & status.eq(STABLE_STATUS) & context_pass] = "stable_structural_accept"
