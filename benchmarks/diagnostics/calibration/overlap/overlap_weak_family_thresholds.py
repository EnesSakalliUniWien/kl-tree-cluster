...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap.overlap_weak_family_thresholds"
...
def build_weak_family_rows(
    ...
    *,
    ...
    working["context_homogeneity_margin"] = pd.to_numeric(
        working["homogeneity_gain_min"], errors="coerce"
    ) - pd.to_numeric(
        ...
        records.append(
            {
                ...
                "truth_aligned_row_count": int(roles.eq("signal_truth_aligned").sum()),
                "truth_misaligned_row_count": int(roles.eq("signal_truth_misaligned").sum()),
