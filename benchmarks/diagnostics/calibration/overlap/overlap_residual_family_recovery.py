...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap.overlap_residual_family_recovery"
...
def build_residual_family_rows(
    ...
    *,
    ...
    joined["continuous_context_margin"] = pd.to_numeric(
        joined["homogeneity_gain_min"], errors="coerce"
        ...
        records.append(
            {
                ...
                "residual_neg_log10_min_sibling_p_value": float(-math.log10(min_p_clipped)),
                "residual_max_homogeneity_gain_min": _max(group["homogeneity_gain_min"]),
                "residual_max_continuous_context_margin": _max(group["continuous_context_margin"]),
                ...
                "residual_min_fragment_risk_proxy_score": _min(group["fragment_risk_proxy_score"]),
                "residual_max_fragment_risk_proxy_score": _max(group["fragment_risk_proxy_score"]),
...
def _comparison_masks(
    ...
    *,
    ...
    if negative_role == "residual_null_or_nonrecovery_family":
        negative = roles.isin({"residual_null_like_family", "residual_nonrecovery_family"})
    else:
            ...
            records.append(
                {
                    ...
                    "positive_retention": (
                        float(positive_selected / positive_total) if positive_total else math.nan
                    ),
                    "negative_selection_rate": (
