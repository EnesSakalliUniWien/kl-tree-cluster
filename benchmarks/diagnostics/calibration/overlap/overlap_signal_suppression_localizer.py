...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap.overlap_signal_suppression_localizer"
...
def _summarize_candidate_nodes(candidates: pd.DataFrame) -> pd.DataFrame:
    ...
    candidates["is_suppressed_candidate"] = candidates["candidate_reason"].isin(SUPPRESSED_REASONS)
    ...
    for keys, group in candidates.groupby(JOIN_KEYS, dropna=False):
        ...
        row: dict[str, object] = {
            ...
            "split_suppressed_by_guard_count": _count_true(group["is_split_suppressed_by_guard"]),
        ...
        for column in NODE_VALUE_COLUMNS:
            values = (
                _to_numeric(suppressed[column])
                if column in suppressed.columns
                else pd.Series(dtype=float)
            )
            row[f"suppressed_median_{column}"] = (
                float(values.median()) if not values.dropna().empty else np.nan
            )
            row[f"suppressed_max_{column}"] = (
                float(values.max()) if not values.dropna().empty else np.nan
            )
        rows.append(row)
...
def build_case_summary(localized: pd.DataFrame) -> pd.DataFrame:
    ...
    for case_id, group in localized.groupby("case_id", dropna=False):
        ...
        row: dict[str, object] = {
            ...
            "split_suppressed_by_guard_count": int(group["split_suppressed_by_guard_count"].sum()),
            "pass_through_suppressed_count": int(group["pass_through_suppressed_count"].sum()),
