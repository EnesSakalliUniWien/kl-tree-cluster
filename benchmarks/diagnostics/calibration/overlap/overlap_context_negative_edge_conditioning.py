...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.overlap.overlap_context_negative_edge_conditioning"
)
...
class OverlapContextNegativeEdgeConditioningConfig:
    ...
    def metric_summary_path(self) -> Path:
        return self.output_dir / "overlap_context_negative_edge_conditioning_metric_summary.csv"
    ...
    def threshold_scan_path(self) -> Path:
        return self.output_dir / "overlap_context_negative_edge_conditioning_threshold_scan.csv"
...
def add_edge_conditioning_metrics(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    enriched["outgoing_minus_incoming_edge_evidence_gap"] = outgoing_min - incoming_max
    enriched["incoming_minus_outgoing_edge_evidence_gap"] = incoming_min - outgoing_max
    incoming_rejections = _bool_series(enriched["incoming_edge_rejected"]).astype(
        int
    ) + _bool_series(enriched["incoming_sibling_edge_rejected"]).astype(int)
    outgoing_rejections = _bool_series(enriched["outgoing_left_edge_rejected"]).astype(
        int
