...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.overlap.overlap_conditional_bayesian_traversal_law"
)
...
class OverlapConditionalBayesianTraversalLawConfig:
    ...
    output_dir: Path
    parameters: ConditionalBayesianTraversalParameters = ConditionalBayesianTraversalParameters()
...
def build_conditional_bayesian_traversal_rows(
    ...
    *,
    ...
    selected_bf = p_value_log_bayes_factor_lower_bound(rows["residual_min_sibling_p_value"])
    ...
    parent_size = _as_numeric(rows, "residual_median_parent_size").fillna(1.0).clip(lower=1.0)
    balance = (
        _as_numeric(rows, "residual_max_barycentric_balance").fillna(0.0).clip(lower=0.0, upper=0.5)
    )
    ...
    family_penalty = float(parameters.family_size_log_penalty_weight) * np.log(family_size)
    selection_context_penalty = parent_penalty + imbalance_penalty + family_penalty
    context_prior = float(parameters.base_log_prior_odds) + depth_credit - selection_context_penalty

    homogeneity_bf = float(parameters.homogeneity_gain_weight) * _as_numeric(
        rows, "residual_max_homogeneity_gain_min"
    ).fillna(0.0)
    context_margin_bf = float(parameters.context_margin_weight) * _as_numeric(
        rows, "residual_max_continuous_context_margin"
    ).fillna(0.0)
    ...
    status.loc[
        ...
        & neighborhood_bf.ge(float(parameters.strong_neighborhood_log_bayes_factor_threshold))
    ...
    status.loc[
        ...
        & neighborhood_bf.lt(float(parameters.strong_neighborhood_log_bayes_factor_threshold))
        ...
        records.append(
            {
                ...
                "coherent_candidate_count": int(status.eq("conditional_coherent_candidate").sum()),
...
def run_overlap_conditional_bayesian_traversal_law(
    ...
    manifest = {
        ...
        "production_status": ("diagnostic_only_no_permutation_no_production_promotion"),
    }
