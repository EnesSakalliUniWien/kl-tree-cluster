...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap.overlap_method_clustering_comparison"
...
def load_assignment_record(path: Path) -> AssignmentRecord:
    ...
    data_role = (
        str(genes["data_role"].iloc[0])
        if "data_role" in genes.columns
        else str(parsed["filename_role"])
    )
    method_id = (
        str(genes["method_id"].iloc[0])
        if "method_id" in genes.columns
        else str(parsed["method_id"])
    )
    replicate = (
        int(genes["replicate"].iloc[0])
        if "replicate" in genes.columns
        else int(parsed["replicate"])
    )
    case_id = (
        str(genes["case_id"].iloc[0]) if "case_id" in genes.columns else str(parsed["case_id"])
    )
        ...
        rows.append(
            {
                ...
                "partition_nmi_between_methods": float(
                    normalized_mutual_info_score(y_left, y_right)
                )
                if len(merged)
                else float("nan"),
