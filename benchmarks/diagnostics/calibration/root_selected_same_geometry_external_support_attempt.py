...
from benchmarks.diagnostics.calibration.root_selected_deformed_mp_edge_panel import (
...
from benchmarks.diagnostics.calibration.root_tie_rank_conditioned_coherent_topology_join import (
    build_conditioned_coherent_joined_feasibility_rows,
)
from benchmarks.diagnostics.calibration.root_tie_rank_generated_neighborhood_replay import (
...
from benchmarks.diagnostics.calibration.root_tie_rank_selected_null_simulation_pilot import (
    DEFAULT_OBSERVED_MIXED_ROWS,
)
from benchmarks.diagnostics.calibration.root_tie_rank_target_conditioned_importance_frontier import (
...
GENERATED_BY = (
    ...
    "root_selected_same_geometry_external_support_attempt"
)
...
ATTEMPT_SUMMARY_OUTPUT = (
    "root_selected_same_geometry_external_support_attempt_summary.csv"
)
...
def _select_external_law_target_case_ids(
    ...
    *,
    ...
    status_mask = rows["external_law_equation_status"].astype(str).isin(
        EXTERNAL_LAW_TARGET_STATUSES
    )
...
def _tail_admissible_joined_rows(
    *,
    ...
    for index, row in rows.iterrows():
        ...
        rows.at[index, "calibration_role"] = (
            f"{old_calibration_role}_h_u_missing_not_tail_support"
        )
...
def _same_tail_support_rows(
    *,
    ...
    candidates = joined_rows.loc[
        ~joined_rows.apply(_is_observed_target, axis=1)
    ].copy()
    if candidates.empty:
        return candidates
...
def _generated_count_by_target(generated_rows: pd.DataFrame) -> dict[str, int]:
    if generated_rows.empty or "conditioning_target_case_id" not in generated_rows.columns:
        return {}
    counts = (
        generated_rows.groupby("conditioning_target_case_id", dropna=False)
        .size()
        .to_dict()
    )
...
def _replay_completed_count_by_target(
    *,
    ...
    if "case_id" not in replay_rows.columns or "case_id" not in generated_rows.columns:
        return {}
    lookup = generated_rows[["case_id", "conditioning_target_case_id"]].drop_duplicates(
        "case_id"
    )
    ...
    counts = (
        completed.groupby("conditioning_target_case_id", dropna=False)
        .size()
        .to_dict()
    )
    return {str(key): int(value) for key, value in counts.items()}
        ...
        records.append(
            {
                ...
                "pre_topology_stratum_hit_count": int(
                    pre_topology_hits.get(target_id, 0)
                ),
...
def summarize_same_geometry_external_support_attempt_rows(
    ...
    calibrated = rows["tail_panel_inference_status"].astype(str).eq(
        "calibrated_selected_root_spectral_tail_available"
    )
    fail_closed = rows["tail_panel_inference_status"].astype(str).eq(
        ...
        [
            {
                ...
                "generated_candidate_count": int(
                    rows["generated_candidate_count"].sum()
                ),
...
def evaluate_same_geometry_external_support_attempt(
    ...
    frontier_tables = evaluate_target_conditioned_importance_frontier(
        TargetConditionedImportanceFrontierConfig(
            ...
            accept_target_pre_topology_stratum=bool(
                config.accept_target_pre_topology_stratum
            ),
    ...
    attempt_summary = summarize_same_geometry_external_support_attempt_rows(
        attempt_rows
    )
    ...
    return {
        ...
        "replay_pvalue_tau_s_sensitivity": replay_tables[
...
def main() -> None:
    ...
    outputs = run_same_geometry_external_support_attempt(
        RootSelectedSameGeometryExternalSupportAttemptConfig(
            ...
            proposal_families=_parse_csv_list(args.proposal_families)
            or DEFAULT_PROPOSAL_FAMILIES,
            two_block_delta_grid=_parse_float_grid(args.two_block_delta_grid),
            spike_feature_fraction_grid=_parse_float_grid(
                args.spike_feature_fraction_grid
            ),
            ...
            accept_target_pre_topology_stratum=bool(
                args.accept_target_pre_topology_stratum
            ),
