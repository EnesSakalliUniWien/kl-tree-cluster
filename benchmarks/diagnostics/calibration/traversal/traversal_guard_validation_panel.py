...
def evaluate_traversal_guard_rows(
    ...
    *,
    ...
    if float(mixed_context_cost_ratio) < 0.0:
        raise ValueError(
            f"mixed_context_cost_ratio must be non-negative; got {mixed_context_cost_ratio!r}."
        )
    missing = REQUIRED_COLUMNS - set(panel.columns)
    if missing:
        raise ValueError(f"Traversal guard panel is missing columns: {sorted(missing)!r}.")
    table = panel.copy()
        ...
        summaries.append(
            {
                ...
                "independent_fraction_threshold": float(first["independent_fraction_threshold"]),
