...
GENERATED_BY = "benchmarks.diagnostics.calibration.traversal.path_conditioned_hypothesis_audit"

DEFAULT_OUTPUT_DIR = Path("raw/assets/benchmark-results/path_conditioned_hypothesis_audit_20260618")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--manual-guarded-dir", type=Path, default=DEFAULT_MANUAL_GUARDED_DIR)
        ...
        rows.append(
            {
                ...
                "pass_through_with_truth_coherent_descendant_rows": int(truth_coherent_desc.sum()),
...
def build_branch_current_table(
    *,
    ...
    if f"{BRANCH_METHOD}_ari" in table.columns and f"{CURRENT_METHOD}_ari" in table.columns:
        table["path_audit_branch_minus_current_ari"] = pd.to_numeric(
            table[f"{BRANCH_METHOD}_ari"], errors="coerce"
    ...
    current_status = table.get(f"{CURRENT_METHOD}_status", pd.Series("", index=table.index))
    branch_status = table.get(f"{BRANCH_METHOD}_status", pd.Series("", index=table.index))
        ...
        rows.append(
            {
                ...
                "branch_pass_through_rows_sum": int(
                    pd.to_numeric(group.get(branch_pass_column, 0), errors="coerce").fillna(0).sum()
                ),
            ...
            rows.append(
                {
                    ...
                    "mean_found_clusters": float(getattr(row, "mean_found_clusters", math.nan)),
