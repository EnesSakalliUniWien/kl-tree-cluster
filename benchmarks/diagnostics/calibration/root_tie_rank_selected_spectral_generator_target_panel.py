...
STUDY_ROLE = (
    "diagnostic_root_tie_rank_selected_spectral_generator_targets_not_calibration"
)
GENERATED_BY = (
    "benchmarks.diagnostics.calibration."
    "root_tie_rank_selected_spectral_generator_target_panel"
)
...
def _eligible_family_rows(
    *,
    ...
    if "conditioning_target_case_id" in rows.columns:
        ...
        rows = rows.loc[
            conditioning_target.astype(str).eq("")
            | conditioning_target.astype(str).eq(target_id)
        ].copy()
...
def _generator_target_record(
    *,
    ...
    calibration_mask = (
        eligible.apply(_is_calibration_support, axis=1)
        if eligible_count
