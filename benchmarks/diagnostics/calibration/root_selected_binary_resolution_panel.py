...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.root_selected_binary_resolution_panel"
)
...
def _json_default(value: object) -> object:
    if isinstance(value, RootSelectedBinaryResolutionConfig):
        return asdict(value)
...
def build_root_selected_binary_resolution_rows(
    *,
    ...
    for _, tail in tail_rows.sort_values("target_case_id").iterrows():
        ...
        bottleneck = (
            min(action, edge)
            if math.isfinite(action) and math.isfinite(edge)
            else math.nan
        )
        strength = (
        ...
        [
            {
                ...
                "transition_resolution_count": int(
                    band.eq("transition_binary_resolution").sum()
                ),
                "strong_resolution_count": int(band.eq("strong_binary_resolution").sum()),
                "existing_tail_support_count": int(
                    status.eq("binary_root_tail_calibrated").sum()
                ),
