...
GENERATED_BY = (
    ...
    "overlap_context_negative_topology_conditioning"
)
...
class OverlapContextNegativeTopologyConditioningConfig:
    ...
    def rows_path(self) -> Path:
        return (
            self.output_dir
            / "overlap_context_negative_topology_conditioning_rows.csv"
        )
    ...
    def metric_summary_path(self) -> Path:
        return (
            self.output_dir
            / "overlap_context_negative_topology_conditioning_metric_summary.csv"
        )
    ...
    def category_summary_path(self) -> Path:
        return (
            self.output_dir
            / "overlap_context_negative_topology_conditioning_category_summary.csv"
        )
    ...
    def summary_path(self) -> Path:
        return (
            self.output_dir
            / "overlap_context_negative_topology_conditioning_summary.csv"
        )
    ...
    def threshold_scan_path(self) -> Path:
        return (
            self.output_dir
            / "overlap_context_negative_topology_conditioning_threshold_scan.csv"
        )
...
def add_topology_conditioning_metrics(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    enriched["selected_family_neglog10_p_value"] = _neglog10_p(
        enriched["selected_family_p_value"]
    )
...
def summarize_context_negative_topology_conditioning(
    ...
    *,
    ...
    category_full = category_rows["category_status"].eq(
        "category_separates_all_truth"
    )
    if bool(full.any()):
...
def run_overlap_context_negative_topology_conditioning(
    ...
    summary, metric_summary, category_summary = (
    ...
    manifest = {
        ...
        "production_status": (
            "diagnostic_only_single_truth_topology_conditioning_scan"
        ),
    }
