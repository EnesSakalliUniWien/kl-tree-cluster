...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.path_conditioned_traversal_audit"
)
...
def add_path_covariates(tuple_rows: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in out.itertuples(index=False):
        ...
        if incoming_parent_row is not None:
            ...
            incoming_parent_sibling_gate_open = _is_true(
                incoming_parent_row.sibling_gate_open
            )
            if str(incoming_parent_row.left_child) == row.node_id:
                incoming_child_side = "left"
                incoming_branch_length = _finite_or_nan(
                    incoming_parent_row.left_branch_length
                )
                ...
                incoming_branch_length = _finite_or_nan(
                    incoming_parent_row.right_branch_length
                )
        ...
        records.append(
            {
                ...
                "median_incoming_branch_length_on_path": (
                    float(np.median(path_branch_lengths))
                    if path_branch_lengths
                    else math.nan
                ),
            }
        )
...
def add_truth_covariates(
    ...
    *,
    ...
    for row in out.itertuples(index=False):
        node_leaves = sorted(
            str(leaf) for leaf in descendant_leaf_sets.get(row.node_id, [])
        )
        left_leaves = sorted(
            str(leaf) for leaf in descendant_leaf_sets.get(row.left_child, [])
        )
        right_leaves = sorted(
            str(leaf) for leaf in descendant_leaf_sets.get(row.right_child, [])
        )
        ...
        records.append(
            {
                ...
                "tuple_majority_truth_count": int(
                    tuple_summary["majority_truth_count"]
                ),
    ...
    out.update(
        {
            ...
            "edge_open_closed_sibling_boundary_rows": int(
                len(edge_open_closed_sibling)
            ),
            "pure_boundary_rows": int(
                (tuple_rows["truth_audit_label"] == PURE_BOUNDARY).sum()
            ),
            "mixed_boundary_rows": int(
                (tuple_rows["truth_audit_label"] == MIXED_BOUNDARY).sum()
            ),
            "truth_coherent_split_path_rows": int(
                (
                    tuple_rows["truth_audit_label"]
                    == TRUTH_COHERENT_SPLIT_PATH
                ).sum()
            ),
            ...
            "pass_through_with_truth_coherent_descendant_split_rows": int(
                (
                    pass_through["descendant_truth_coherent_live_split_count"]
                    > 0
                ).sum()
            ),
        }
        ...
        rows.append(
            {
                ...
                "stacked_pass_through_rows": int(
                    (group["ancestor_pass_through_count"] > 0).sum()
                ),
                ...
                "median_incoming_branch_length": float(
                    group["incoming_branch_length"].median()
                ),
                "rows_with_truth_coherent_descendant_split": int(
                    (
                        group["descendant_truth_coherent_live_split_count"]
                        > 0
                    ).sum()
                ),
                "truth_coherent_pass_through_rows": int(
                    (
                        group["truth_audit_label"]
                        == TRUTH_COHERENT_SPLIT_PATH
                    ).sum()
                ),
            }
        ...
        rows.append(
            {
                ...
                "pure_boundary_rows": int(
                    (group["truth_audit_label"] == PURE_BOUNDARY).sum()
                ),
                "mixed_boundary_rows": int(
                    (group["truth_audit_label"] == MIXED_BOUNDARY).sum()
                ),
                "edge_open_closed_sibling_boundary_rows": int(
                    len(edge_open_closed)
                ),
...
def _truth_by_leaf_from_computed(computed: Any) -> dict[str, object]:
    ...
    return {
        str(sample_id): labels[index]
        for index, sample_id in enumerate(computed.data.index)
    }
...
def run_path_conditioned_traversal_audit(
    ...
    for case_idx, case in enumerate(cases, start=1):
        ...
        for method_id in methods:
            ...
            for params in spec.param_grid:
                ...
                descendant_leaf_sets = computed.tree.compute_descendant_sets(
                    use_labels=True
                )
    ...
    all_tuples = (
        pd.concat(tuple_frames, ignore_index=True)
        if tuple_frames
        else pd.DataFrame()
    )
    ...
    outputs = {
        ...
        "path_conditioned_pass_through_summary": output_dir
        / PASS_THROUGH_SUMMARY_OUTPUT,
