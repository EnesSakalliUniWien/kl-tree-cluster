...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap_income_outcome_junction_law"
        ...
        records.append(
            {
                ...
                "parent_id": (
                    str(row["parent_id"]) if pd.notna(row["parent_id"]) else ""
                ),
                ...
                "outgoing_depth": (
                    float(row["outgoing_depth"])
                    if pd.notna(row["outgoing_depth"])
                    else np.nan
                ),
                "outgoing_structural_decision_zone": (
                ...
                "outgoing_fragment_risk_proxy_score": float(
                    row["fragment_risk_proxy_score"]
                ),
        ...
        [
            {
                ...
                "truth_transition_required_count": int(
                    (truth & transition_required).sum()
                ),
