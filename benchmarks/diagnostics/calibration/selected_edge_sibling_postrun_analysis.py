...
from benchmarks.diagnostics.calibration.selected_edge_sibling_null_equation import (
...
def build_distribution_shape_records(siblings: pd.DataFrame) -> pd.DataFrame:
    ...
    if "covariance_inferred_reference_scale" in source.columns:
        records["alternate_reference_scale"] = source[
        ...
        records.append(
            {
                ...
                "notes": (
                    "Current projected-Wald distribution shape for selected-edge "
                    "sibling rows."
                ),
            }
        )
        ...
        records.append(
            {
                ...
                "required_for_production": True,
                "notes": (
                    "Conditional empirical selected edge+sibling equation support."
                ),
            }
        )
