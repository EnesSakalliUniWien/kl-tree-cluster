...
def summarize_edge_sibling_pairs(pair_table: pd.DataFrame) -> pd.DataFrame:
    ...
    for group_key, group in pair_table.groupby(group_columns, dropna=False):
        ...
        finite = group[
            group["edge_pair_max_neglog10_bh"].notna()
            & group["sibling_neglog10_corrected"].notna()
        ]
        spearman = (
            float(
        ...
        rows.append(
            {
                ...
                "min_sibling_corrected_p_value": float(
                    group["sibling_p_value_corrected"].min()
                ),
                ...
                "median_sparse_sibling_neglog10": float(
                    group["sibling_sparse_neglog10"].median()
                ),
                "median_dense_sibling_neglog10": float(
                    group["sibling_dense_neglog10"].median()
                ),
...
def plot_edge_sibling_relationship(
    ...
    *,
    ...
    for test_index, test_type in enumerate(test_types):
        for role_index, role in enumerate(roles):
            values = long_table.loc[
                long_table["test_type"].eq(test_type)
                & long_table["data_role"].eq(role),
                "neglog10_p_value",
            ].dropna()
            if values.empty:
                continue
    ...
    ratio_rows = pair_table[
        pair_table["sibling_to_edge_max_neglog10_ratio"].replace(
            [np.inf, -np.inf],
            np.nan,
        ).notna()
    ].copy()
