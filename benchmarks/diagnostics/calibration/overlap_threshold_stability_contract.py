...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap_threshold_stability_contract"
...
def _validate_inputs(hierarchy: pd.DataFrame, transfer_summary: pd.DataFrame) -> None:
    ...
    if transfer_missing:
        raise ValueError(
            f"Threshold transfer summary is missing columns: {transfer_missing!r}"
        )
...
def _classify_stage(
    ...
    negative_rate = (
        float(stage["negative_rate"]) if pd.notna(stage["negative_rate"]) else 0.0
    )
        ...
        rows.append(
            {
                ...
                "transfer_checked": bool(transfer["transfer_checked"]),
                "transfer_leakage_detected": bool(
                    transfer["transfer_leakage_detected"]
                ),
                ...
                "transfer_truth_recovery_retention": transfer[
                    "transfer_truth_recovery_retention"
                ],
                "transfer_null_like_leakage_rate": transfer[
                    "transfer_null_like_leakage_rate"
                ],
                "transfer_nonrecovery_leakage_rate": transfer[
                    "transfer_nonrecovery_leakage_rate"
                ],
                "notes": notes,
            }
