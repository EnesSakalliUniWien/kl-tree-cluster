...
from benchmarks.diagnostics.calibration.selected_tail_admissibility_domain import (
    SelectedTailRun,
    parse_run_specs,
)
...
def _validate_tail_law_table(table: pd.DataFrame, *, path: Path) -> None:
    ...
    if missing:
        raise ValueError(
            f"Selected-tail law table {path} is missing columns: {sorted(missing)!r}."
        )
...
def evaluate_q5_promotion_gate(
    ...
    *,
    ...
    max_split_residual_tail_abs_error: float = (
        DEFAULT_MAX_Q5_SPLIT_RESIDUAL_TAIL_ABS_ERROR
    ),
    ...
    rows.loc[bad_error, "q5_split_failure_reason"] = (
        "residual_tail_abs_error_above_gate"
    )
    missing_error = ~np.isfinite(rows["residual_tail_exceedance_absolute_error"])
    rows.loc[missing_error, "q5_split_failure_reason"] = (
        "missing_residual_tail_abs_error"
    )
...
def _support_contract_met(row: pd.Series) -> bool:
    return bool(
        int(row["n_matching_simulations"])
        >= int(row["required_min_matching_simulations"])
...
def _context_tail_abs_error_met(
    ...
    *,
    ...
    return bool(
        ...
        and absolute_error <= float(max_context_heldout_tail_abs_error)
    )
...
def evaluate_selected_tail_promotion_gate(
    ...
    *,
    ...
    max_q5_split_residual_tail_abs_error: float = (
        DEFAULT_MAX_Q5_SPLIT_RESIDUAL_TAIL_ABS_ERROR
    ),
    max_context_heldout_tail_abs_error: float = (
        DEFAULT_MAX_CONTEXT_HELDOUT_TAIL_ABS_ERROR
    ),
...
def run_selected_tail_promotion_gate(
    *,
    ...
    manifest: Mapping[str, object] = {
        ...
        "study_role": STUDY_ROLE,
        "run_specs": [
            {"run_id": run.run_id, "selected_tail_law_csv": str(run.path)}
            for run in runs
        ],
        "q5_validation_path": str(q5_validation_path),
        "q5_model_id": q5_model_id,
        "max_q5_split_residual_tail_abs_error": float(
            max_q5_split_residual_tail_abs_error
        ),
        "max_context_heldout_tail_abs_error": float(
            max_context_heldout_tail_abs_error
        ),
        "require_c_hat_precision": bool(require_c_hat_precision),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    outputs = run_selected_tail_promotion_gate(
        ...
        max_q5_split_residual_tail_abs_error=float(
            args.max_q5_split_residual_tail_abs_error
        ),
