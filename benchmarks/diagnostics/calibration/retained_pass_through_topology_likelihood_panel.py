...
GENERATED_BY = (
    ...
    "retained_pass_through_topology_likelihood_panel"
)
...
def build_retained_pass_through_topology_likelihood_rows(
    ...
    for _, row in selected.iterrows():
        ...
        if traversal_network_context_rows is None:
            ...
            pass_through_distance = _finite_float(
                context_row["distance_to_pass_through_context"]
            )
        ...
        records.append(
            {
                ...
                "left_neg_log10_sibling_p_value": _neg_log10_p(
                    row["left_sibling_p_value"]
                ),
                ...
                "left_outgoing_edge_norm_balance": row[
                    "left_outgoing_edge_norm_balance"
                ],
                ...
                "match_eligible": bool(
                    _is_finite(row["left_depth"]) and math.isfinite(log_leaves)
                ),
            }
        )
    ...
    *,
    ...
    for _, signal_row in signal.iterrows():
        ...
        for _, control_row in controls.iterrows():
            ...
            if (
                depth_delta > float(max_depth_delta)
                or log_delta > float(max_log_descendant_leaves_delta)
            ):
                continue
            ...
            ) and _is_finite(control_row["distance_to_downstream_accepted_split"]):
                split_delta = abs(
            ...
            records.append(
                {
                    ...
                    "signal_log_descendant_leaves": float(
                        signal_row["left_log_descendant_leaves"]
                    ),
                    ...
                    "tree_network_distance_status": (
                        "no_matched_selected_null_control"
                    ),
        ...
        records.append(
            {
                ...
                "control_log_descendant_leaves": float(
                    best_control["left_log_descendant_leaves"]
                ),
...
def summarize_retained_pass_through_topology_likelihood(
    ...
    *,
    ...
    unmatched = match_rows.loc[
        match_rows["match_status"].eq("no_matched_selected_null_control")
    ]
    matched_control_finite = int(
        matched["control_has_finite_topology"].astype(bool).sum()
    )
    ...
    finite_signal_edge = int(
        signal["finite_outgoing_edge_norm_balance"].astype(bool).sum()
    )
    finite_control_edge = int(
        controls["finite_outgoing_edge_norm_balance"].astype(bool).sum()
    )
    ...
    matched_traversal_context = int(
        matched["traversal_network_match_status"]
        .eq("matched_traversal_network_context")
        .sum()
    )
    cross_case_network_unavailable = int(
        matched["tree_network_distance_status"]
        .eq("cross_case_network_distance_unavailable")
        .sum()
    )
    context_statuses = set(
        likelihood_rows["traversal_network_context_status"].dropna().astype(str)
    )
        ...
        [
            {
                ...
                "median_match_distance": _finite_median(
                    match_rows["match_distance"]
                ),
                "finite_signal_pass_through_context_count": (
                    finite_signal_pass_context
                ),
                "finite_control_pass_through_context_count": (
                    finite_control_pass_context
                ),
                "finite_signal_downstream_split_distance_count": (
                    finite_signal_downstream_split
                ),
                "finite_control_downstream_split_distance_count": (
                    finite_control_downstream_split
                ),
                "matched_traversal_network_context_count": (
                    matched_traversal_context
                ),
                "cross_case_network_distance_unavailable_count": (
                    cross_case_network_unavailable
                ),
...
def run_retained_pass_through_topology_likelihood_panel(
    ...
    match_rows = build_retained_pass_through_topology_match_rows(
        ...
        max_downstream_accepted_split_delta=(
    ...
    manifest = {
        ...
        "matching": {
            ...
            "max_log_descendant_leaves_delta": float(
                config.max_log_descendant_leaves_delta
            ),
            "max_pass_through_context_delta": float(
                config.max_pass_through_context_delta
            ),
            "max_downstream_accepted_split_delta": float(
                config.max_downstream_accepted_split_delta
            ),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    outputs = run_retained_pass_through_topology_likelihood_panel(
        RetainedPassThroughTopologyLikelihoodConfig(
            ...
            max_log_descendant_leaves_delta=float(
                args.max_log_descendant_leaves_delta
            ),
            max_pass_through_context_delta=float(
                args.max_pass_through_context_delta
            ),
            max_downstream_accepted_split_delta=float(
                args.max_downstream_accepted_split_delta
            ),
