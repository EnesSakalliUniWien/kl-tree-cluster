    ...
    *,
    ...
    if status.tested and not status.ancestor_blocked:
        if not np.isfinite(status.p_value_bh):
            raise ValueError(
                "Tested non-blocked child edges require finite Tree-BH p-values; "
                f"child={child!r}."
            )
    ...
    ancestor_kernel = float(
        np.exp(
            -max(ancestor_support.distance - 1.0, 0.0)
            / kernel_bandwidths.ancestor_distance
        )
    )
    ...
    support = float(
        (
            ancestor_kernel * ancestor_support.p_value_bh
            + stable_mass * stable_support
        )
        / support_denominator
    )
...
def audit_case(case: dict[str, object]) -> CaseAuditTables:
    ...
    summary_row = {
        ...
        "n_positive_df_records": int(
            sum(record.degrees_of_freedom > 0.0 for record in records)
        ),
        ...
        "n_selected_nonnull_positive_weight_records": int(
            np.sum(selected_nonnull_positive_mask)
        ),
        ...
        "n_selected_nonnull_with_interpolated_weight": int(
            np.sum(
                selected_nonnull_positive_mask
                & np.isfinite(weights)
                & (weights > 0.0)
            )
        ),
        ...
        "n_dimensionless_stable_reference_edges": len(
            reference_sets.dimensionless_stable_nodes
        ),
