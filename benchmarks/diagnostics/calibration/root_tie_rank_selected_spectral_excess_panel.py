...
GENERATED_BY = (
    ...
    "root_tie_rank_selected_spectral_excess_panel"
)
...
def _json_default(value: object) -> object:
    if isinstance(value, RootTieRankSelectedSpectralExcessConfig):
        return asdict(value)
...
def _eligible_rows(
    *,
    ...
    rows = generated.copy()
    rows["_spectral_excess_log"] = rows[
...
def _spectral_law_record(
    *,
    ...
    calibration_mask = eligible.apply(_is_calibration_support, axis=1) if eligible_count else pd.Series(dtype=bool)
    ...
    return {
        ...
        "required_spectral_excess_multiplier": (
            float(math.exp(spectral_deficit))
            if math.isfinite(spectral_deficit)
            else math.nan
        ),
...
def summarize_selected_spectral_excess_rows(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, group in rows.groupby(
        ...
        calibration_supported = int(
            group["eligible_calibration_support_count"].gt(0).sum()
        )
        diagnostic_best = int(
