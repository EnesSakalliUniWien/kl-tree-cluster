...
def _stratum_summary(
    ...
    *,
    ...
    for group_values, group in records.groupby(group_columns, dropna=False):
        ...
        simulation_means = group.groupby("replicate_index")["selected_hierarchy_ratio"].mean()
        ...
        simulation_se = (
            float(
                ...
                / np.sqrt(n_matching_simulations)
            )
        ...
        row.update(
            {
                ...
                "reference_expectation_median": float(np.quantile(reference_expectations, 0.5)),
