...
class ExternalCalibrationContract:
    ...
    def resolved_min_matching_simulations(self) -> int:
        ...
        return int(ceil(1.0 / (self.sibling_alpha * self.tail_resolution_fraction_of_alpha)) - 1)
...
def _simulation_relative_se(records: pd.DataFrame, ratios: np.ndarray) -> float:
    simulation_means = records.groupby("replicate_index")["selected_hierarchy_ratio"].mean()
    ...
    if c_hat == 0.0:
        return np.nan
    se = float(
        np.std(simulation_means.to_numpy(dtype=float), ddof=1) / np.sqrt(simulation_means.shape[0])
    )
...
def _admissibility_failure_reasons(
    *,
    ...
    if n_matching_simulations < contract.resolved_min_matching_simulations():
        reasons.append("matching_simulations_below_tail_resolution_contract")
    if n_records < contract.resolved_min_matched_records():
        reasons.append("matched_records_below_tail_resolution_contract")
    if not np.isfinite(relative_c_se) or relative_c_se > contract.max_relative_c_simulation_se:
...
def _scalar_c_summary(
    ...
    *,
    ...
    scalar_rejection_rate = float(np.mean(scalar_p_values <= contract.sibling_alpha))
    scalar_threshold_ratios = (
        c_hat * chi2.isf(contract.sibling_alpha, df=degrees_of_freedom) / degrees_of_freedom
    )
    selected_ratio_tail_quantile = float(np.quantile(ratios, 1.0 - contract.sibling_alpha))
    ...
    tail_quantile_excess = float(selected_ratio_tail_quantile / scalar_tail_threshold_ratio)
    scalar_rejection_limit = contract.sibling_alpha + contract.scalar_rejection_absolute_tolerance
    ...
    row.update(
        {
            ...
            "required_min_matching_simulations": (contract.resolved_min_matching_simulations()),
            ...
            "required_max_relative_c_simulation_se": (contract.max_relative_c_simulation_se),
...
def evaluate_external_calibration_contract(
    ...
    *,
    ...
    if missing_columns:
        raise ValueError(
            f"Selected-hierarchy contract records are missing columns: {sorted(missing_columns)}."
        )
    if records.empty:
...
def main() -> None:
    ...
    outputs = run_external_calibration_contract_diagnostic(
        ...
        seed=int(args.seed),
        contract=ExternalCalibrationContract(
            tail_resolution_fraction_of_alpha=float(args.tail_resolution_fraction_of_alpha),
            ...
            scalar_rejection_absolute_tolerance=float(args.scalar_rejection_absolute_tolerance),
            scalar_tail_quantile_excess_limit=float(args.scalar_tail_quantile_excess_limit),
            min_scalar_uniformity_ks_p_value=float(args.min_scalar_uniformity_ks_p_value),
        ),
    )
