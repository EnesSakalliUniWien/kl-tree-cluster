...
def prepare_q5_selected_tail_records(records: pd.DataFrame) -> pd.DataFrame:
    ...
    table[RESPONSE_COLUMN] = pd.to_numeric(table[RESPONSE_COLUMN], errors="raise").astype(float)
    table["replicate_index"] = pd.to_numeric(table["replicate_index"], errors="raise").astype(int)
    ...
    modeling_columns = [
        ...
        *[
            ...
            if column.startswith("feature_family__") and not column.endswith("__baseline")
        ],
        *[
            column
            for column in table.columns
            if column.startswith("parent_size_bin__") and not column.endswith("__baseline")
        ],
    ]
...
def q5_model_specs(model_table: pd.DataFrame) -> tuple[ModelSpec, ...]:
    ...
    return (
        ...
        ModelSpec(
            ...
            description=(
                "log_R ~ edge_action + log(lambda_k/lambda_MP) + spectral_mass + effective_rank"
            ),
...
def _binary_auc_score(scores: np.ndarray, labels: np.ndarray) -> float:
    ...
    return float(
        (rank_sum_positive - n_positive * (n_positive + 1) / 2.0) / (n_positive * n_negative)
    )
...
def _fit_tail_model(
    ...
    *,
    ...
    finite_train = (
        train[list(predictors) + [response]]
        .replace(
            [np.inf, -np.inf],
            np.nan,
        )
    ...
    active_predictors = tuple(
        predictor for predictor in predictors if finite_train[predictor].nunique(dropna=True) > 1
    )
    minimum_rows = max(
