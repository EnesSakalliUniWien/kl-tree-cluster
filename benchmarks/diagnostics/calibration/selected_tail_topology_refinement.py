...
def parse_records_specs(raw_specs: list[str]) -> tuple[RecordsInput, ...]:
    ...
    for raw_spec in raw_specs:
        if "=" not in raw_spec:
            raise ValueError(
                "Selected-geometry record specs must use run_id=csv_path; "
                f"got {raw_spec!r}."
            )
...
def summarize_refinement_families(tail_laws: pd.DataFrame) -> pd.DataFrame:
    ...
    for context_family, group in tail_laws.groupby("context_family", dropna=False):
        descriptive = group[
            group["tail_law_status"].eq("descriptive_holdout_tail_law")
        ]
...
def compare_refinements_to_base(
    ...
    *,
    ...
    rows: list[dict[str, object]] = []
    for context_family, group in tail_laws[
        ~tail_laws["context_family"].eq("base")
    ].groupby("context_family", dropna=False):
        for context_key, refined_group in group.groupby(list(base_context_columns), dropna=False):
            ...
            admissible = refined_group[
                refined_group["production_tail_law_admissible"].astype(bool)
            ]
            ...
            if not diagnostic_passed.empty:
                interpretation = (
                    "refinement_has_diagnostic_contract_pass_not_production"
                )
            ...
            elif np.isfinite(base_se) and np.isfinite(best_refined_se) and best_refined_se < base_se:
            ...
            row = {
                column: value
                for column, value in zip(base_context_columns, context_key)
            }
            row.update(
                {
                    "context_family": context_family,
                    ...
                    "base_heldout_exceedance_rate": float(
                        base_row["heldout_exceedance_rate"]
                    ),
                    ...
                    "n_refined_production_admissible_contexts": int(
                        admissible.shape[0]
                    ),
    ...
    return pd.DataFrame.from_records(rows).sort_values(
        ["context_family", *base_context_columns]
