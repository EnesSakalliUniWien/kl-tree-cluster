...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap_threshold_hierarchy"
...
def build_overlap_threshold_hierarchy(
    ...
    stable_negative = _int(stable["null_count"]) + _int(
        stable["signal_truth_misaligned_count"]
    )
    records.append(
        {
            "stage_order": 1,
    ...
    threshold_rows = {
        str(row["threshold_name"]): row for _, row in residual_thresholds.iterrows()
    }
    threshold_specs = (
        (
            3,
    ...
    records.append(
        {
            ...
            "target_total": int(
                residual_eligibility_summary["truth_recovery_family_count"].sum()
            ),
            "target_rate": _rate(
            ...
            "negative_total": int(
                residual_eligibility_summary["nonrecovery_family_count"].sum()
            ),
            "negative_rate": _rate(
...
def run_overlap_threshold_hierarchy(
    ...
    manifest = {
        ...
        "residual_eligibility_summary_path": str(
            config.residual_eligibility_summary_path
        ),
