...
GENERATED_BY = "benchmarks.diagnostics.calibration.root_selected_deformed_mp_edge_panel"
...
def _deformed_mp_z(v_value: float, spectrum: np.ndarray, aspect_ratio: float) -> float:
    return float(
        -1.0 / v_value
        + aspect_ratio * np.mean(spectrum / (1.0 + spectrum * v_value))
    )
...
def _deformed_mp_edge_derivative(
    ...
    return float(
        1.0 / (v_value * v_value)
        - aspect_ratio
        * np.mean((spectrum * spectrum) / ((1.0 + spectrum * v_value) ** 2))
    )
...
def build_root_selected_deformed_mp_edge_rows(
    *,
    ...
    h_u_by_case = {
        ...
        for _, row in h_u_observability_rows.iterrows()
    }
        ...
        records.append(
            {
                ...
                "s_root_identity_excess_log": _positive_log(
                    float(components["identity_ratio"])
                ),
                "s_root_deformed_excess_log": _positive_log(
                    float(components["deformed_ratio"])
                ),
        ...
        records.append(
            {
                ...
                "production_inference_status": (
                    ...
                    if _string_value(row, "calibration_role")
        ...
        [
            {
                ...
                "missing_input_count": int(
                    status.eq("deformed_mp_edge_missing_inputs").sum()
                ),
                ...
                "max_deformed_excess_log": float(excess.max())
                if computed_count
                else math.nan,
                "summary_status": (
