...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.root_selected_kernel_spectral_tail_law_panel"
)
...
DEFAULT_DEFORMED_MP_EDGE_ROWS = (
    DEFAULT_INPUT_ROOT / "root_selected_deformed_mp_edge_rows.csv"
)
...
def _fill_observed_root_topology(
    *,
    ...
    if "case_id" not in observed_root_summary_rows.columns or "case_id" not in rows.columns:
        return rows
    summary_by_case = {
        str(row["case_id"]): row for _, row in observed_root_summary_rows.iterrows()
    }
    enriched = rows.copy()
    ...
    *,
    ...
    if coarse:
        ...
        parts["tie_cell_presence"] = (
            "tie_cell_present"
            if _finite_float(row.get("root_child_discrete_tie_cell_count", math.nan))
            > 0.0
            else "tie_cell_absent"
        )
        parts["smooth_presence"] = (
            "smooth_present"
            if _finite_float(row.get("root_child_smooth_constraint_count", math.nan))
            > 0.0
            else "smooth_absent"
        )
        parts.pop("smooth")
...
def _prepare_joined_rows(
    ...
    rows["s_h_u_deformed_available"] = rows["case_id"].astype(str).map(
        lambda case_id: math.isfinite(deformed_lookup.get(case_id, math.nan))
    )
    return rows
...
def _strict_tail_lookup(strict_tail_rows: pd.DataFrame) -> dict[str, pd.Series]:
    if strict_tail_rows.empty or "target_case_id" not in strict_tail_rows.columns:
        return {}
    return {
        str(row["target_case_id"]): row for _, row in strict_tail_rows.iterrows()
    }
    ...
    *,
    ...
    if not bool(np.any(finite)):
        return {
            ...
            "positive_count": int(pd.to_numeric(support["s_h_u_excess_log"], errors="coerce").gt(0.0).sum()),
            "exceedance_count": 0,
            "weight_sum": 0.0,
            "effective_n": 0.0,
            "max_share": math.nan,
    ...
    if not bool(np.any(keep)):
        return {
            ...
            "positive_count": int(pd.to_numeric(valid_support["s_h_u_excess_log"], errors="coerce").gt(0.0).sum()),
            "exceedance_count": 0,
            "weight_sum": 0.0,
            "effective_n": 0.0,
            "max_share": math.nan,
        ...
        records.append(
            {
                ...
                "target_s_h_u_excess_log": _finite_float(
                    target.get("s_h_u_excess_log", math.nan)
                ),
                "target_spectral_tail_variable": (
                    "deformed_mp_s_h_u"
                ...
                "target_root_topology_signature": topology_summary[
                    "target_signature"
                ],
                ...
                "kernel_weighted_exceedance_fraction": support_summary[
                    "weighted_exceedance"
                ],
                ...
                "topology_coarsened_support_count": topology_summary[
                    "coarsened_count"
                ],
                "topology_admissible_support_count": topology_summary["support_count"],
                "topology_positive_s_h_u_support_count": topology_summary[
                    "positive_count"
                ],
                ...
                "topology_weighted_exceedance_fraction": topology_summary[
                    "weighted_exceedance"
                ],
                ...
                "topology_nearest_support_case_id": topology_summary[
                    "nearest_case_id"
                ],
                "topology_nearest_support_weight_share": topology_summary[
                    "nearest_weight_share"
                ],
                "topology_nearest_support_s_h_u_excess_log": topology_summary[
                    "nearest_s"
                ],
...
def summarize_kernel_spectral_tail_rows(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    topology_available = topology_decision.eq(
    ...
    summary = {
        ...
        "topology_support_missing_count": int(
            topology_status.eq("topology_support_missing").sum()
        ),
        "topology_degenerate_support_count": int(
            (
                topology_status.eq("topology_kernel_weight_too_concentrated")
                | topology_status.eq(
                    "topology_kernel_effective_support_insufficient"
                )
            ).sum()
        ),
