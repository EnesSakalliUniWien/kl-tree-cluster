...
from benchmarks.diagnostics.calibration.robust_root_center_smoke import (
...
GENERATED_BY = "benchmarks.diagnostics.calibration.robust_center_rooted_tree_smoke"
...
def build_unrooted_linkage_tree(matrix: np.ndarray) -> LinkageTree:
    ...
    leaf_sets: dict[int, frozenset[int]] = {
        index: frozenset({index}) for index in range(n_leaves)
    }
    coordinates: dict[int, np.ndarray] = {
        index: matrix[index].copy() for index in range(n_leaves)
    }
...
def run_robust_center_rooted_tree_smoke(
    ...
    summary["root_edge_true_distance_rank"] = summary.groupby(
        ["dimension", "scenario"]
    )["root_edge_true_distance_mean"].rank(method="dense")
