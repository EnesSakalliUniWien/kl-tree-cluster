...
GENERATED_BY = (
    ...
    "root_selected_deformed_external_law_target_panel"
)
...
def _json_default(value: object) -> object:
    if isinstance(value, RootSelectedDeformedExternalLawTargetConfig):
        return asdict(value)
...
def build_root_selected_deformed_external_law_target_rows(
    ...
    for _, row in support_gap_rows.sort_values("target_case_id").iterrows():
        ...
        conditioning_event = (
            "R_root_selected"
            f" AND stratum={_string_value(row, 'target_root_tail_stratum_key')}"
        )
        records.append(
            {
        ...
        [
            {
                ...
                "existing_support_count": int(
                    status.eq("existing_tail_support_available").sum()
                ),
