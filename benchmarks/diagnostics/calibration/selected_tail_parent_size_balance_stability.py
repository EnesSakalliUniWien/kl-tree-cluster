...
def prepare_parent_size_balance_records(records: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(
            "Parent-size balance stability records are missing columns: "
            f"{sorted(missing)!r}."
        )
    table = records.copy()
    ...
    table["log_barycentric_leverage"] = np.log(
        np.maximum(beta_left, beta_right) / balance
    )
    table["barycentric_balance_bin"] = [
        _balance_bin(float(value)) for value in balance
    ]
    ...
    *,
    ...
    for parent_bin in parent_bins:
        ...
        if status == "ok":
            ...
            fold_exceedances = (
                test["selected_hierarchy_ratio"].to_numpy(dtype=float) > threshold
            )
            ...
            exceedance_se = float(
                np.sqrt(
                    exceedance_rate
                    * (1.0 - exceedance_rate)
                    / max(fold_exceedances.shape[0], 1)
                )
            )
        fold_rows.append(
            {
...
def evaluate_parent_size_balance_stability(
    ...
    *,
    ...
    for context_values, group in table.groupby(list(CONTEXT_COLUMNS), dropna=False):
        ...
        c_hat_precision_met = bool(
            np.isfinite(relative_c_se)
            and relative_c_se <= float(max_relative_c_simulation_se)
        )
        parent_size_holdout_valid = bool(
            parent_size_summary["n_used_parent_size_folds"] >= 2
        )
        parent_size_abs_error = float(
            parent_size_summary["parent_size_heldout_exceedance_absolute_error"]
        )
        parent_size_se = float(
            parent_size_summary["parent_size_heldout_exceedance_standard_error"]
        )
        parent_size_abs_error_met = bool(
        ...
        parent_size_precision_met = bool(
            np.isfinite(parent_size_se)
            and parent_size_se <= max_exceedance_standard_error
        )
        ...
        output.update(
            {
                ...
                "required_max_relative_c_simulation_se": float(
                    max_relative_c_simulation_se
                ),
                "c_hat_precision_met": c_hat_precision_met,
        ...
        rows.append(
            {
                ...
                "n_parent_size_holdout_valid": int(
                    group["parent_size_holdout_valid"].sum()
                ),
                "n_parent_size_abs_error_met": int(
                    group["parent_size_abs_error_met"].sum()
                ),
                "n_parent_size_precision_met": int(
                    group["parent_size_precision_met"].sum()
                ),
                "study_role": STUDY_ROLE,
            }
        )
