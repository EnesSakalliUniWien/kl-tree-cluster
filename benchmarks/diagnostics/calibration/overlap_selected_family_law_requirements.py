...
GENERATED_BY = "benchmarks.diagnostics.calibration.overlap_selected_family_law_requirements"
...
def _validate_inputs(
    ...
    missing_family = sorted(
        {"residual_family_truth_role", *CONDITIONING_METRICS}
        - set(residual_family_rows.columns)
    )
    if missing_family:
...
def build_selected_family_law_requirements(
    ...
    nontransferable_stages = stability_rows[
        stability_rows["threshold_stability_status"].astype(str).eq(
