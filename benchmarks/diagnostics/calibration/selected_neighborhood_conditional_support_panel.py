...
GENERATED_BY = (
    ...
    "selected_neighborhood_conditional_support_panel"
)
...
def _root_context(
    *,
    ...
    validity_status = str(
        validity_record.get("root_validity_status", "root_validity_unmeasured")
    )
    validity_tail_status = str(
        validity_record.get("root_tail_inference_status", "")
    )
...
def _spectral_flow_pass(row: pd.Series, *, require_spectral_flow: bool) -> bool:
    if not bool(require_spectral_flow):
        return True
    return (
        _string_value(row, "spectral_bottleneck_status")
        == "spectral_flow_observed_diagnostic_only"
    )
...
def build_conditional_support_rows(
    ...
    *,
    ...
    if missing:
        raise ValueError(
            f"measurability rows missing required columns: {sorted(missing)!r}."
        )
    ...
    for _, row in measurability_rows.iterrows():
        ...
        effective_support = _finite_float(
            row.get("interpolation_effective_support", math.nan)
        )
        ...
        records.append(
            {
                ...
                "direct_sibling_open": _bool_value(
                    row.get("direct_sibling_open", False)
                ),
        ...
        records.append(
            {
                ...
                "root_tail_inference_status": _dominant(
                    group["root_tail_inference_status"]
                ),
                "selected_root_usability_status": _dominant(
                    group["selected_root_usability_status"]
                ),
                "root_gate_allows_neighborhood": _bool_sum(
                    group["root_gate_allows_neighborhood"]
                )
                > 0,
...
def summarize_conditional_support(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    root_failed_cases = root_context_by_case[
        root_context_by_case["root_context_status"].astype(str).eq(
            "root_validity_failed"
        )
    ]
...
def evaluate_selected_neighborhood_conditional_support(
    ...
    root_tail_rows = (
        ...
        if config.root_tail_rows_path is not None
        and Path(config.root_tail_rows_path).exists()
        else None
    )
...
def run_selected_neighborhood_conditional_support(
    ...
    rows, case_summary, summary = evaluate_selected_neighborhood_conditional_support(
        config
    )
