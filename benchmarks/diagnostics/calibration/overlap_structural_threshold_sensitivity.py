...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.overlap_structural_threshold_sensitivity"
)
    ...
    *,
    ...
    return {
        ...
        "threshold_status": _threshold_status(
            ...
            blocked_signal_truth_aligned_count=blocked_signal_truth_aligned,
            same_subspace_heterogeneous_warning_count=(
                same_subspace_heterogeneous_warning
            ),
            unrelated_subspace_signal_count=unrelated_subspace_signal,
        ),
    }
...
def summarize_threshold_recommendations(sensitivity: pd.DataFrame) -> pd.DataFrame:
    ...
    for key_values, group in sensitivity.groupby(keys, sort=True):
        ...
        signal_truth_aligned_accept = int(
            signal_rows["signal_truth_aligned_accept_count"].sum()
        )
        ...
        heterogeneity_warnings = int(
            group["same_subspace_heterogeneous_warning_count"].sum()
        )
        ...
        signal_truth_aligned_case_replicate_total = int(
            signal_truth_aligned_by_row.gt(0).sum()
        )
        ...
        null_rate = (
            null_false_accept / null_accepted_total
            if null_accepted_total > 0
            else 0.0
        )
        retention = (
        ...
        records.append(
            {
                ...
                "signal_truth_misaligned_accept_count": (
                    signal_truth_misaligned_accept
                ),
