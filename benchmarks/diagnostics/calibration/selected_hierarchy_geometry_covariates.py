...
def _edge_float(edge_df: pd.DataFrame, node_id: object, column_name: str) -> float:
    ...
    if not np.isfinite(value):
        raise ValueError(
            f"Edge value {column_name!r} for child {node_id!r} must be finite; "
            f"got {value!r}."
        )
    return value
...
def _log_selected_hierarchy_ratio(ratio: float) -> tuple[float, str]:
    if ratio < 0.0 or not np.isfinite(ratio):
        raise ValueError(f"selected_hierarchy_ratio must be finite and non-negative; got {ratio!r}.")
    if ratio == 0.0:
...
def _spectral_geometry(
    *,
    ...
    if eigenvalues.ndim != 1:
        raise ValueError(
            f"Eigenvalues for parent {parent!r} must be 1-D; got {eigenvalues.shape}."
        )
    ...
    if projection_dimension < eigenvalues.shape[0] and eigenvalues[projection_dimension] > 0.0:
        eigengap = float(
            eigenvalues[projection_dimension - 1] / eigenvalues[projection_dimension]
        )
    ...
    selected_eigenvalue_over_mp, selected_eigenvalue_over_mp_status = (
        _defined_ratio_or_nan(
...
def _selected_geometry_rows(
    *,
    ...
    for record in records:
        ...
        child_size_ratio = float(max(left_sample_size, right_sample_size) / min(left_sample_size, right_sample_size))
        ...
        row = {
            ...
            "negative_log10_min_child_edge_bh_p_value": _defined_log10_p_value(
                min_edge_bh
            ),
            "left_edge_tested": _edge_bool(edge_df, record.left, "Child_Parent_Divergence_Tested"),
            "right_edge_tested": _edge_bool(edge_df, record.right, "Child_Parent_Divergence_Tested"),
            "left_edge_significant": _edge_bool(edge_df, record.left, "Child_Parent_Divergence_Significant"),
            "right_edge_significant": _edge_bool(edge_df, record.right, "Child_Parent_Divergence_Significant"),
...
def _holdout_summary(
    *,
    ...
    non_tail_score_mean = (
        float(np.mean(predicted[~labels])) if np.any(~labels) else np.nan
    )
    return {
        "equation_id": equation_id,
        "equation": equation,
...
def _candidate_equation_table(records: pd.DataFrame) -> pd.DataFrame:
    ...
    table["feature_parent_aspect_ratio"] = (
        table["feature_dimension"].astype(float) / table["parent_sample_size"].astype(float)
    )
    table["sampling_variance_scale"] = (
        1.0 / table["left_child_sample_size"].astype(float)
        ...
        rows.append(
            {
                ...
                "selected_eigenvalue_mass_fraction_q95": float(
                    np.quantile(eigen_mass, 0.95)
                ),
                "study_role": STUDY_ROLE,
            }
        )
