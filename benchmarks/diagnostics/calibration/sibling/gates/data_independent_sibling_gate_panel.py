...
GENERATED_BY = (
    "benchmarks.diagnostics.calibration.sibling.gates.data_independent_sibling_gate_panel"
)
...
def validate_selected_topology_penalties(penalties: Sequence[float]) -> tuple[float, ...]:
    ...
    if invalid:
        raise ValueError(
            f"selected_topology_penalties must be finite and positive; got {invalid!r}."
        )
...
def _generate_data(
    *,
    ...
    if data_role == "signal":
        ...
        raise ValueError("Signal diagnostics require a FeatureSpace for non-binary cases.")
                ...
                rows.append(
                    {
                        ...
                        "rejected_at_sibling_alpha": bool(p_value <= float(sibling_alpha)),
        ...
        row.update(
            {
                ...
                "rejection_rate_at_sibling_alpha": float(group["rejected_at_sibling_alpha"].mean()),
                "rejection_rate_at_effective_alpha": float(
                    group["rejected_at_effective_alpha"].mean()
                ),
                "effective_selected_alpha": float(group["effective_selected_alpha"].iloc[0]),
                "p_value_q50": float(p_values.quantile(0.50)),
                "p_value_q05": float(p_values.quantile(0.05)),
                "parent_sample_size_q90": float(group["parent_sample_size"].quantile(0.90)),
                "large_parent_rejection_rate_at_effective_alpha": float(
                    group.loc[
                        group["parent_sample_size"] >= group["parent_sample_size"].quantile(0.90),
...
def _transfer_status(group: pd.DataFrame) -> str:
    ...
    null_ok = null_rows["data_independent_gate_status"].eq("data_independent_gate_null_candidate")
...
def build_data_independent_gate_components(
    ...
    for _, row in summary.iterrows():
        ...
        context = f"case_id={row['case_id']}|data_role={row['data_role']}|{contract_context}"
        records.append(
            {
                "contract_id": (f"data_independent_sibling_gate_candidate:{contract_context}"),
    ...
    if transfer_summary is not None:
        for _, row in transfer_summary.iterrows():
            ...
            records.append(
                {
                    "contract_id": (f"data_independent_sibling_gate_candidate:{context}"),
                    "component_id": f"data_independent_gate_transfer:{context}",
                    "component_type": "data_independent_sibling_gate_transfer",
                    "component_status": str(row["data_independent_gate_transfer_status"]),
                    "required_for_production": True,
