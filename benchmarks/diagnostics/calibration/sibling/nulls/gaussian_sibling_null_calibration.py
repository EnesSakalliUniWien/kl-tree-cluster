...
def validate_gaussian_sibling_null_context(
    ...
    if context.projection_dimension <= 0:
        raise ValueError("Gaussian sibling null context requires positive projection_dimension.")
...
def append_external_gaussian_null_columns(
    ...
    if missing_calibrations:
        raise ValueError(f"Calibration table is missing columns: {sorted(missing_calibrations)}.")
