...
def _family_baseline_inflation(
    ...
    reference_expectations = (
        model.sample_reference_scales[mask] * (model.sample_degrees_of_freedom[mask])
    )
    return float(
        max(
...
def _variant_result(
    *,
    ...
    selected_records = tuple(
        record for record, selected in zip(family_records, selected_mask, strict=True) if selected
    )
...
def _require_trace_columns(trace_df: pd.DataFrame) -> None:
    ...
    if missing:
        raise ValueError(
            f"Sibling inflation diagnostic trace is missing columns: {sorted(missing)!r}."
        )
        ...
        rows.append(
            {
                ...
                "contributor_projection_dimension": float(record.sibling_projection_dimension),
                ...
                "weighted_statistic_contribution": float(weighted_statistics[local_index]),
                "weighted_reference_contribution": float(weighted_references[local_index]),
        ...
        rows.append(
            {
                ...
                "n_corrected_p_blocks": int(group["annotation_corrected_blocks_at_alpha"].sum()),
                ...
                "n_strict_empirical_null_supported": int(
                    (group["calibration_support_status"] == "strict_empirical_null_supported").sum()
                ),
                ...
                "min_inflation_excess_ratio": float(group["inflation_excess_ratio"].min()),
...
def build_sibling_inflation_diagnostic_tables(
    *,
    ...
    focal_records = [
        record for record in records if not record.is_null_like and record.degrees_of_freedom > 0.0
    ]
    for record in focal_records:
        ...
        include_contributors = bool(target["blocker_candidate"]) or (
            ...
            and (bool(target["inflation_crosses_alpha"]) or bool(target["fdr_crosses_alpha"]))
        )
