...
CONTINUOUS_LOCAL_EDGE_SELECTION_SCOPE = "continuous_fixed_projection_local_edge_selection"
...
def validate_selection_conditioned_context(
    ...
    if context.standardized_contrast_dimension <= 0:
        raise ValueError(
            "Selection-conditioned context requires positive standardized_contrast_dimension."
        )
...
def simulate_local_edge_selection_conditioned_null(
    ...
    *,
    ...
    conditional_mean_over_reference = float(conditional_mean / context.reference_expectation)
    scaled_observed_statistic = float(
        context.observed_statistic / (context.reference_scale * conditional_mean_over_reference)
    )
    return SelectionConditionedSiblingNullResult(
        ...
        empirical_tail_p_value=float(
            (1 + np.count_nonzero(statistics >= context.observed_statistic)) / (accepted_count + 1)
        ),
        mean_scaled_chi2_p_value=float(
