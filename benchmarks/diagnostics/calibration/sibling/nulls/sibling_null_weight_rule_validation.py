...
def _probability_series(values: pd.Series, *, column_name: str) -> pd.Series:
    ...
    if bool(((numeric < 0.0) | (numeric > 1.0) | ~np.isfinite(numeric)).any()):
        bad_index = numeric[((numeric < 0.0) | (numeric > 1.0) | ~np.isfinite(numeric))].index[0]
        raise ValueError(
    ...
    for rule_id in WEIGHT_RULE_IDS:
        ...
        if label_masks is None:
            ...
            labeled_metrics = {
                ...
                "n_supported_positive_weight_records": int(np.sum(positive & support)),
                ...
                "selected_nonnull_weight_share": (
                    selected_nonnull_weight_sum / weight_sum if weight_sum > 0.0 else np.nan
                ),
