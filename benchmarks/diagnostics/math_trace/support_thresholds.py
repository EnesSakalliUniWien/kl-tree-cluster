...
def audit_support_table(
    ...
    checks = {
        ...
        "min_family_supported_records": (
            _column_or_default(table, "n_family_supported_records", thresholds.min_family_supported_records)
        ...
        "min_n_eff_family": (
            _column_or_default(table, "n_eff_family", 0.0)
            >= thresholds.min_n_eff_family
        ),
        ...
        "max_weight_share": (
            _column_or_default(table, "max_weight_share", 0.0)
    ...
    result["support_status"] = result[check_names].all(axis=1).map(
