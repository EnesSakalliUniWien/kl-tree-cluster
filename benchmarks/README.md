...
python benchmarks/diagnostics/analyze_relationships.py
```
...
recoverability, gate-path tracing, and sibling-calibration diagnostic tools.
...
For a focused diagnosis of the current KL gap on categorical and overlapping families, see `benchmarks/diagnostics/categorical_overlapping_gap_diagnosis.md`.
...
`benchmarks/diagnostics/` for benchmark-level investigations and
`benchmarks/calibration/` for edge-calibration helpers. The full benchmark
runner only runs the clustering comparison and relationship analysis.
