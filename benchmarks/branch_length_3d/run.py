...
def run_3d_benchmark(
    ...
    for n_features in feature_counts:
        for rep in range(n_replicates):
            ...
            if verbose:
                print(
                    f"[{current}/{total}] Features={n_features}, Replicate {rep + 1}/{n_replicates}"
                )
...
def compute_grid_summary(combined: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = (
        combined.groupby(["branch_length", "n_features"])
        .agg(
            {
                "ari": ["mean", "std"],
                "nmi": ["mean", "std"],
...
def plot_line_by_features(summary: pd.DataFrame, n_leaves: int) -> plt.Figure:
    ...
    ax.set_title(
        f"ARI vs Branch Length by Feature Count\n({n_leaves} leaves)",
        fontsize=14,
        weight="bold",
    )
...
def plot_correct_split_rate(combined: pd.DataFrame, n_leaves: int) -> plt.Figure:
    ...
    ax.set_title(
        f"Rate of Finding Correct 2-Cluster Split\n({n_leaves} leaves)",
        fontsize=14,
        weight="bold",
    )
    ...
    plt.colorbar(im, ax=ax, label="Correct Split Rate")
