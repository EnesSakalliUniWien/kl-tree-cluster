...
| Method                                                    | What it does                                                                                                                                                                                                                                                                         |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `__init__(tree, annotations_df, ...)`                     | Configure thresholds, pre-cache node metadata, run `_prepare_annotations()`, build fast lookup dicts for gate decisions.                                                                                                                                                             |
| `_prepare_annotations(annotations_df)`                    | If gate columns are missing, runs Gate 2 (`annotate_child_parent_divergence`) then Gate 3 (sibling test selected by `config.SIBLING_TEST_METHOD`). Idempotent.                                                                                                                       |
...
| `_should_split_v2(parent)`                                | **Enhanced gate logic**. Same gates, but when Gate 3 passes, runs `localize_divergence_signal()` to find WHERE divergence originates. Returns `(bool, LocalizationResult)`. **Known bug**: returns `(True, loc_result)` even when post-FDR localization finds zero difference pairs. |
...
| `decompose_tree_v2()`                                     | Same traversal with `_should_split_v2()`. Builds similarity/difference graphs, extracts constrained clusters, applies post-hoc merge. **Known bug**: `merge_points` ancestors can overlap leaf sets with localization-graph descendant nodes.                                        |
...
| `extract_constrained_clusters(sim_graph, diff_graph, tree, merge_points)`        | Union-Find merge of similar pairs, respecting Cannot-Link constraints from difference graph. **Known bug**: leaf overlap when `merge_points` are ancestors of graph nodes.                      |
