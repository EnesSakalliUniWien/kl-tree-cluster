...
- The default sibling gate remains `projected_wald_inflation`. Opt-in
  ...
  `root_selective_permutation_guard_replicates` exposes the default-off
  selected-root permutation guard for fixed-subspace sibling gates; when
  enabled, it records `Root_Selective_Permutation_*` diagnostics and closes an
  open root whose selected-root p-value is above the guard alpha. The guard
  scope defaults to `root`; diagnostic profiles can also request selected
  subtree checks for `open_internal` nodes or for the narrower
  `passthrough_descendant` contexts that are reachable only through an
  ordinary closed sibling ancestor. The
...
- `sibling_gate_profile="fixed_coordinate_selective_root_v1"` packages the
  current stronger non-cross-fit candidate: fixed coordinate BH,
  selected-topology alpha penalty, selected-root stability, and the
  selected-root permutation guard. It remains a diagnostic candidate, not a
  default.
...
- `sibling_gate_profile="fixed_coordinate_global_passthrough_v1"` is the
  ...
  conservative global selected-family sibling-min guard for pass-through
  descendants and remains validation-only because broad signal sensitivity and
  runtime cost are not yet production-certified.
