...
def plot_branch_length_results(df: pd.DataFrame, title: str = "Branch Length Benchmark") -> plt.Figure:
    ...
    ax.plot(df["branch_length"], df["n_clusters_found"], "r-o", label="Found", linewidth=2, markersize=8)
    ax.set_xlabel("Branch Length (evolution steps)", fontsize=11)
    ...
    ax = axes[1, 1]
    scatter = ax.scatter(df["js_divergence"], df["ari"], c=df["branch_length"], 
...
def plot_embedding_by_branch_length(
    ...
    for idx, bl in enumerate(branch_lengths):
        ...
        rng = np.random.RandomState(random_seed + bl if random_seed else None)
        ...
        if use_umap:
            reducer = umap.UMAP(n_components=2, random_state=42, 
                               n_neighbors=min(15, len(X_scaled) - 1), min_dist=0.1)
        ...
        for label in [0, 1]:
            ...
            ax_true.scatter(embedding[mask, 0], embedding[mask, 1], 
                           c=colors_true[label], label=label_name, alpha=0.7, s=50, 
        ...
        if show_kl_clustering:
            ...
            pred_labels, n_found, status = _run_clustering(data_df, "kl", {}, random_seed)
            ...
            if pred_labels is not None and status == "ok":
                ...
                for i, label in enumerate(unique_labels):
                    ...
                    color = cmap(i % 10)
                    ax_kl.scatter(embedding[mask, 0], embedding[mask, 1],
                                 c=[color], label=f"C{label}", alpha=0.7, s=50,
                                 edgecolors="black", linewidth=0.5)
                
                ari = adjusted_rand_score(true_labels, pred_labels)
                ax_kl.set_title(f"KL Clustering (BL={bl})\nFound {n_found}, ARI={ari:.2f}", 
