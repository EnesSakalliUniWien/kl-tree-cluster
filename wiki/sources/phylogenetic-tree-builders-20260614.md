...
updated: 2026-06-14
sources:
  - kl_clustering_analysis/tree/phylogenetic.py
...
The KL runner now has opt-in tree estimators for neighbor joining and IQ-TREE 3.
...
- `kl_neighbor_joining` builds a neighbor-joining tree from the configured KL
  tree distance vector and roots the resulting unrooted metric tree with MAD.
- `kl_iqtree3` writes the feature matrix as a binary/morphological-state
...
- The default benchmark method set is unchanged. IQ-TREE remains optional
  ...
  encoding is a modeling choice, not a default KL-TE contract.
...
- `kl_clustering_analysis/tree/phylogenetic.py` implements
...
- `benchmarks/shared/runners/method_registry.py` registers
  `kl_neighbor_joining` and `kl_iqtree3` as explicit opt-in methods.
- `benchmarks/shared/runners/kl_runner.py` keeps current linkage behavior for
  `kl` while routing the new methods through the shared `PosetTree` and gate
  pipeline.
