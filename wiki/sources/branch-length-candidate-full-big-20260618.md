...
This run compares `tbs`, `tbs_legacy_c2ef9a69`, and
`tbs_internal_filter_branch_length_v1` across the full `121`-case benchmark
...
does not promote it over current `tbs`. Current TBS has the best completed-row
...
- `tbs` records `93` OK rows, `28` skips, `66` exact-K rows, mean ARI
...
- `tbs_legacy_c2ef9a69` records `121` OK rows, no skips, `78` exact-K rows,
  mean ARI `0.726685`, and median ARI `0.994656`.
- The relationship report identifies current `tbs` as the best average-ARI
...
- On `overlap_extreme_4c`, current `tbs` and branch-length skip for missing
...
- `benchmark_relationship_report.md` records full-suite section summaries and
  model contrasts. It analyzes `305` completed rows and reports current `tbs`
  as best average ARI.
