...
over comparable Gaussian and categorical source families. It is diagnostic
only: admissible rows identify contexts where the selected-tail support
contract is met, while all non-admissible contexts remain undefined for
...
- Four contexts are production-admissible under the diagnostic support
  contract:
  `gaussian_blobs` and `categorical_multinomial`, both in
  `small_0_0.25`, `edge_action_ge8`, with sibling projection dimensions `1`
  and `2`.
...
- The current admissibility domain is therefore not "Gaussian only." It is
  small-parent, high-edge-action selected-tail contexts for Gaussian and
  categorical source families in this targeted panel. Binary, continuous,
  root, medium-parent, large-parent, lower-edge-action, and precomputed-distance
  contexts remain outside the validated production domain.
...
- `raw/assets/benchmark-results/selected_tail_admissibility_domain_20260603/context_admissibility_domain.csv`
  records admissibility status for broad, focused, and boundary runs.
- `raw/assets/benchmark-results/selected_tail_admissibility_domain_20260603/nearest_boundary_contexts.csv`
