...
binary working representation makes one-hot categorical dependence and
continuous-discretization choices part of the statistical problem, not merely
preprocessing details. Second, the local Marchenko--Pastur rule, the minimum
projection dimension, and the sibling projection rule are working spectral
rules whose finite-sample behavior must be validated in the regimes used by the
method. Third, the context-weighted empirical-null inflation is an empirical
calibration model rather than an exact selective-inference theorem. Fourth, the
final partition depends on the pass-through traversal rule, so full-pipeline
