...
feature-space contract. Binary and one-hot cases use Hamming distance and
average linkage for the candidate tree; explicit continuous benchmark cases use
the recorded continuous distance matrix.
...
covariance matrix for that block. Historical binary benchmarks still use
preprocessing maps into a discrete representation. One basic option is median
binarization,
\begin{equation}
X_{ij}^{\mathrm{bin}}
=
...
the testing logic reads the same node-distribution and feature-space contract;
the tree-distance contract is recorded separately for binary, categorical, and
