...
Tree-Break Selection itself is the procedure
\[
\operatorname{TBS}(X,\mathcal T,\mathcal F;
\alpha_{\mathrm{edge}},\alpha_{\mathrm{sib}})
\longmapsto
\mathcal C_{\mathrm{TBS}},
\]
where \(\mathcal F\) is the feature-space contract and
\(\mathcal C_{\mathrm{TBS}}\) is the returned set of terminal cluster nodes.
\Cref{tab:notation} summarizes the symbols that carry the rest of the method.

{\small
\begin{longtable}{@{}p{0.24\textwidth}p{0.70\textwidth}@{}}
\caption{Core notation used throughout the Tree-Break Selection method description.}
\label{tab:notation}\\
\toprule
Symbol & Meaning \\
\midrule
...
$\mathcal F$ & Feature-space contract declaring Bernoulli, categorical, continuous, or mixed covariance blocks. \\
\(\operatorname{TBS}(X,\mathcal T,\mathcal F;\alpha_{\mathrm{edge}},\alpha_{\mathrm{sib}})\) & Tree-Break Selection procedure applied to a feature matrix, candidate hierarchy, feature-space contract, and edge/sibling testing levels. \\
...
$t_{u,c}$ & Normalized branch time for edge $(u,c)$, with linkage-derived root-to-leaf paths summing to one. \\
$\bar t$ & Mean positive branch time in the inferred tree, used only to form dimensionless branch-time variance multipliers. \\
...
$s_u^{\mathrm{sib}}=(1/n_{l(u)}+1/n_{r(u)})(1+\{t_{u,l(u)}+t_{u,r(u)}\}/(2\bar t))$ & Branch-time-inflated variance scale for the sibling contrast at parent $u$. \\
...
$B_{\mathrm{TBS}}(u)$ & Tree-break indicator; equals one when TBS accepts the proposed split at internal node $u$. \\
...
$\mathcal{C}_{\mathrm{TBS}}$ & Final set of terminal cluster nodes returned by TBS traversal. \\
\end{longtable}
}
