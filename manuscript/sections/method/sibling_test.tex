...
parent leaf count. The method uses the working post-selection null family
...
\(\hat c_{f_u}(x_u)\), and it is not currently used as a hard threshold for
accepting or rejecting calibration support.
...
strict-null-like or stopped by the edge path.
...
stopped-or-null support. If both are empty but $\mathcal C_u$ has positive
...
calibration contract is unsatisfied. A leave-one-context version of this
support contract may be needed for a stronger finite-sample guarantee, but it
is not the current implementation and is therefore left as validation work.
