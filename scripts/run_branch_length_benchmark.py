...
Usage:
    python scripts/run_branch_length_benchmark.py
"""
...
def run_replicated_benchmark(
    ...
    if branch_lengths is None:
        branch_lengths = list(range(1, 31))
...
def compute_summary_stats(combined: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = combined.groupby("branch_length").agg({
        "ari": ["mean", "std"],
        "nmi": ["mean", "std"],
        "js_divergence": "mean",
    ...
    summary.columns = [
        "branch_length", "ari_mean", "ari_std", "nmi_mean", "nmi_std",
        "js_div_mean", "n_clusters_mean", "n_clusters_std"
    ]
...
def plot_averaged_results(summary: pd.DataFrame, n_replicates: int, n_leaves: int) -> plt.Figure:
    ...
    ax = axes[0, 0]
    ax.errorbar(summary["branch_length"], summary["ari_mean"], yerr=summary["ari_std"],
                fmt="b-o", capsize=3, label="ARI", linewidth=2, markersize=6)
    ax.errorbar(summary["branch_length"], summary["nmi_mean"], yerr=summary["nmi_std"],
                fmt="g-s", capsize=3, label="NMI", linewidth=2, markersize=6)
    ax.set_xlabel("Branch Length (evolution steps)", fontsize=11)
    ax.set_ylabel("Score", fontsize=11)
    ax.set_title(f"Clustering Performance (mean ± std, n={n_replicates})", fontsize=12, weight="bold")
    ...
    ax.errorbar(summary["branch_length"], summary["n_clusters_mean"], yerr=summary["n_clusters_std"],
                fmt="r-o", capsize=3, label="Found", linewidth=2, markersize=6)
    ax.set_xlabel("Branch Length (evolution steps)", fontsize=11)
    ...
    # JS Divergence
    ax = axes[1, 0]
    ax.plot(summary["branch_length"], summary["js_div_mean"], "m-o", linewidth=2, markersize=6)
    ax.set_xlabel("Branch Length (evolution steps)", fontsize=11)
    ...
    ax = axes[1, 1]
    scatter = ax.scatter(summary["js_div_mean"], summary["ari_mean"], 
                         c=summary["branch_length"], cmap="viridis", 
    ...
    fig.suptitle(f"Branch Length Benchmark ({n_replicates} replicates, {n_leaves} leaves fixed)",
...
def main():
    ...
    print(f"Running {n_replicates} replicates for branch lengths 1-30...")
    print()
    ...
    print("=" * 70)
    print()
    print(summary[["branch_length", "js_div_mean", "n_clusters_mean", "ari_mean", "ari_std"]].to_string(index=False))
    ...
    fig3 = plot_embedding_by_branch_length(
        n_leaves=n_leaves,
        n_features=n_features,
        branch_lengths=[1, 5, 10, 15, 20, 25],
        mutation_rate=mutation_rate,
    ...
    print("=" * 70)
    print()
    print(f"- Tested branch lengths: 1-30")
