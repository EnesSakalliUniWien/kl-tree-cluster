...
Usage:
    python scripts/run_multi_split_benchmark.py
"""
...
def _jukes_cantor_transition_matrix(n_categories: int, branch_length: float) -> np.ndarray:
...
def _generate_ancestral_sequence(n_features: int, n_categories: int, rng: np.random.RandomState) -> np.ndarray:
    """Generate a random ancestral sequence."""
    return rng.randint(0, n_categories, size=n_features)


def _evolve_sequence(ancestor: np.ndarray, branch_length: float, n_categories: int, rng: np.random.RandomState) -> np.ndarray:
    ...
    for i, state in enumerate(ancestor):
        evolved[i] = rng.choice(n_categories, p=P[state])
    return evolved
...
def generate_multi_group_data(
    ...
    root = _generate_ancestral_sequence(n_features, n_categories, rng)
    ...
    for g in range(n_groups):
        ...
        group_ancestor = _evolve_sequence(root, between_group_branch, n_categories, rng)
        
        # Generate samples for this group
        for i in range(n_samples_per_group):
            name = f"G{g}_S{i}"
            sample_dict[name] = _evolve_sequence(group_ancestor, within_group_branch, n_categories, rng)
...
def _run_clustering(
    ...
    try:
        if method_id == "kl":
            result = spec.runner(
                data_df, distance_condensed, config.SIBLING_ALPHA,
...
def run_multi_split_benchmark(
    ...
    for n_groups in n_groups_list:
        ...
        for rep in range(n_replicates):
            ...
            # Run clustering
            pred_labels, n_found, status = _run_clustering(data_df, "kl", {}, base_seed + rep)
...
def plot_multi_split_results(df: pd.DataFrame) -> plt.Figure:
    ...
    summary = df.groupby("n_groups_true").agg({
        "ari": ["mean", "std"],
        "nmi": ["mean", "std"],
        "n_groups_found": ["mean", "std"],
        "correct_k": "mean",
    }).reset_index()
    summary.columns = ["n_groups", "ari_mean", "ari_std", "nmi_mean", "nmi_std", 
                       "k_found_mean", "k_found_std", "correct_k_rate"]
    
    # Plot 1: ARI and NMI vs number of groups
    ax = axes[0, 0]
    ax.errorbar(summary["n_groups"], summary["ari_mean"], yerr=summary["ari_std"],
                fmt='-o', label="ARI", linewidth=2, markersize=8, capsize=4)
    ax.errorbar(summary["n_groups"], summary["nmi_mean"], yerr=summary["nmi_std"],
                fmt='-s', label="NMI", linewidth=2, markersize=8, capsize=4)
    ...
    ax = axes[0, 1]
    ax.errorbar(summary["n_groups"], summary["k_found_mean"], yerr=summary["k_found_std"],
                fmt='-o', color='green', linewidth=2, markersize=8, capsize=4, label="Found")
    ax.plot(summary["n_groups"], summary["n_groups"], 'k--', linewidth=2, label="True (diagonal)")
    ax.set_xlabel("True Number of Groups", fontsize=12)
    ...
    ax = axes[1, 0]
    bars = ax.bar(summary["n_groups"], summary["correct_k_rate"], color='steelblue', edgecolor='black')
    ax.set_xlabel("True Number of Groups", fontsize=12)
    ...
    for bar, val in zip(bars, summary["correct_k_rate"]):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.02, 
                f'{val:.0%}', ha='center', va='bottom', fontsize=10, weight='bold')
...
def plot_multi_split_heatmap(df: pd.DataFrame, branch_lengths: List[float], n_groups_list: List[int]) -> plt.Figure:
    """Plot heatmap of ARI across branch lengths and group counts."""
    # Pivot to create grid
    pivot_ari = df.pivot_table(index="n_groups_true", columns="between_branch", values="ari", aggfunc="mean")
    pivot_correct = df.pivot_table(index="n_groups_true", columns="between_branch", values="correct_k", aggfunc="mean")
    ...
    # Correct K heatmap
    ax = axes[1]
    im = ax.imshow(pivot_correct.values, cmap='RdYlGn', aspect='auto', vmin=0, vmax=1, origin='lower')
    ...
    cbar = plt.colorbar(im, ax=ax)
    cbar.set_label("Correct K Rate", fontsize=11)
    ...
    for i in range(len(pivot_correct.index)):
        for j in range(len(pivot_correct.columns)):
            ...
            color = 'white' if val < 0.5 else 'black'
            ax.text(j, i, f'{val:.0%}', ha='center', va='center', color=color, fontsize=8, weight='bold')
...
def main():
    ...
    summary = combined.groupby(["between_branch", "n_groups_true"]).agg({
        "ari": "mean",
        "correct_k": "mean",
        "n_groups_found": "mean",
    }).round(2)
