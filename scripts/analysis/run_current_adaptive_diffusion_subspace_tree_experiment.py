...
from scripts.analysis.go_ic_tree_summary_plots import (
    cluster_coherence,
...
def write_workflow_pdfs(output_dir: Path, ranking: pd.DataFrame, artifact_prefix: str) -> dict[str, str]:
    ...
    with PdfPages(pdf_path) as pdf:
        for row in ok.itertuples(index=False):
            ...
            _image_panel(axes[0], getattr(row, "subspace_embedding_annotated_terms_png", ""), "Subspace embedding")
            _image_panel(
                axes[1],
            ...
            if loadings_path.exists():
                ...
                ax = fig.add_subplot(111)
                _image_panel(ax, getattr(row, "axis_terms_combined_png", ""), f"{title}: GO-term loadings")
                fig.tight_layout()
            page_path = pages_dir / (
                f"{rank:02d}_current__adaptive_diffusion_cosine_subspace__"
    ...
    (method_dir / "README.md").write_text(
        "\n".join(
            [
                ...
                f"- Readable duplicate: `{readable_pdf.name}`",
    ...
    (plots_dir / "README.md").write_text(
        "\n".join(
            [
                ...
                "- Ordered tree/subspace pages PDF: `all_tree_pages_ordered_by_go_ic.pdf`",
...
def write_artifact_index(
    ...
    lines = [
        ...
        f"- [readable method PDF]({Path(workflow_outputs['readable_method_pages_pdf']).relative_to(output_dir)})",
        f"- [all tree-pages PDF]({Path(workflow_outputs['all_tree_pages_pdf']).relative_to(output_dir)})",
        f"- [ordered tree-pages PDF]({Path(workflow_outputs['ordered_tree_pages_pdf']).relative_to(output_dir)})",
        "",
        "## Subspaces",
        "",
