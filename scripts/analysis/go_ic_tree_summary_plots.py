...
def candidate_subspace_and_tree(
    ...
    if tree_geometry == "whole_adaptive_diffusion":
        distances, _metadata = _build_adaptive_diffusion_distance(
            data,
            k_neighbors=15,
            diffusion_time=3,
            n_components=30,
...
def draw_ranking_plots(ranking: pd.DataFrame, output_dir: Path, top_n: int) -> None:
    ...
    fig.tight_layout()
    fig.savefig(output_dir / "go_ic_top_tree_ranking.png", dpi=180)
    ...
    scatter_plot(
        ranking,
        output_dir / "go_ic_quality_scatter.png",
...
def main() -> None:
    ...
    ranking_path = args.output_dir / "go_ic_tree_ranking.csv"
    ranking.to_csv(ranking_path, index=False)
    ranking.to_csv(args.output_dir / f"{RESULT_PREFIX}_tree_ranking.csv", index=False)
    pd.concat(coherence_frames, ignore_index=True).to_csv(
        args.output_dir / "go_ic_cluster_coherence_long.csv",
        index=False,
    )
    shutil.copyfile(
        args.output_dir / "go_ic_cluster_coherence_long.csv",
    ...
    pd.concat(tfidf_frames, ignore_index=True).to_csv(
        args.output_dir / "go_ic_tfidf_quality_long.csv",
        index=False,
    )
    shutil.copyfile(
        args.output_dir / "go_ic_tfidf_quality_long.csv",
    ...
    with PdfPages(pdf_path) as pdf:
        for artifact in ordered_artifacts:
            ...
            pdf.savefig(fig)
            plt.close(fig)
    shutil.copyfile(pdf_path, args.output_dir / f"{RESULT_PREFIX}_all_tree_pages.pdf")
