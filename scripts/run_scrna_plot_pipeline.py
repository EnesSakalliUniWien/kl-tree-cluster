...
def _python_script(name: str, *args: str) -> list[str]:
    return [sys.executable, str(PROJECT_ROOT / "scripts" / name), *args]


def _r_script(name: str, *args: str) -> list[str]:
    return ["Rscript", str(PROJECT_ROOT / "scripts" / name), *args]
...
def pipeline_commands(dataset: str, output_dir: Path) -> list[list[str]]:
    ...
    commands = [
        _python_script(
            "plot_pancreas_all_method_umap_clusters.py",
            "--output-dir",
            output_arg,
            "--title",
            title,
        ),
        _r_script("plot_pancreas_tbs_radial_trees_ggtree.R", output_r_arg),
        _r_script(
            "plot_pancreas_tbs_umap_tree_combo_ggtree.R",
            output_r_arg,
            f"--title={combo_title}",
        ),
    ]
    if dataset == "pancreas":
        commands.insert(
            1,
            _python_script(
                "plot_pancreas_tbs_readable_umap_clusters.py",
                "--output-dir",
                output_arg,
            ),
        )
        commands.insert(
            3,
            _r_script("plot_pancreas_tbs_cluster_radial_trees_ggtree.R", output_r_arg),
        )
    if dataset == "goncalves":
        commands.extend(
            [
                _python_script("analyze_goncalves_tbs_progenitors.py", "--output-dir", output_arg),
                _r_script(
                    "plot_goncalves_tbs_progenitor_trees_ggtree.R",
                    output_r_arg,
                    "--pdf-width=30",
                    "--pdf-height=42",
                    "--page-width=30",
...
def add_known_rows(dataset: str, output_dir: Path, rows: dict[str, dict[str, Any]]) -> None:
    ...
    for name in ["qc_metric_distributions.png", "classical_umap_overview.png"]:
        add(
            ...
            role="canonical",
            canonical=True,
            generator_script="scripts/pancreas_scrna_cluster_benchmark.py",
    ...
    for suffix in ["png", "pdf"]:
        add(
            ...
            role="canonical",
            canonical=True,
            generator_script="scripts/plot_pancreas_all_method_umap_clusters.py",
            input_tables=common_inputs,
        ...
        add(
            ...
            role="canonical",
            canonical=True,
            generator_script="scripts/plot_pancreas_tbs_umap_tree_combo_ggtree.R",
    ...
    if dataset == "pancreas":
        for suffix in ["png", "pdf"]:
            add(
                ...
                role="derivative",
                canonical=False,
                generator_script="scripts/plot_pancreas_tbs_readable_umap_clusters.py",
        ...
        for stem in [
            ...
            for suffix in ["png", "pdf"]:
                add(
                    ...
                    role="derivative",
                    canonical=False,
                    generator_script="scripts/analyze_pancreas_tbs_inner_nodes.py;scripts/compare_pancreas_tbs_to_progenitor_signatures.py",
    ...
    if dataset == "goncalves":
        for stem in [
            ...
            for suffix in ["png", "pdf"]:
                add(
                    ...
                    role="canonical",
                    canonical=True,
                    generator_script="scripts/analyze_goncalves_tbs_progenitors.py",
        ...
        add(
            ...
            role="canonical",
            canonical=True,
            generator_script="scripts/plot_goncalves_tbs_progenitor_trees_ggtree.R",
    ...
    *,
    ...
    if path.name.endswith("_clusters_radial_tree_ggtree.png") or path.name.endswith(
        ...
        return base_row(
            ...
            role="derivative" if status == "present" else "orphaned_previous_run",
            canonical=False,
            generator_script="scripts/plot_pancreas_tbs_cluster_radial_trees_ggtree.R",
    ...
    if path.name.endswith("_radial_tree_ggtree.png") or path.name.endswith("_radial_tree_ggtree.pdf"):
        ...
        return base_row(
            ...
            role="derivative" if status == "present" else "orphaned_previous_run",
            canonical=False,
            generator_script="scripts/plot_pancreas_tbs_radial_trees_ggtree.R",
            input_tables=f"{prefix}_tree_edges.csv",
