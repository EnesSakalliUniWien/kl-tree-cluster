...
Usage:
    python scripts/run_temporal_benchmark.py
"""
...
def plot_temporal_results(df, title="Incremental Temporal Clustering"):
    ...
    # ARI vs Divergence
    ax = axes[1, 1]
    ax.scatter(df["divergence_from_ancestor"], df["ari"], c=df["time_step"], 
...
def main():
    ...
    for name, df in [("Fast", df_fast), ("Moderate", df_moderate), ("Slow", df_slow)]:
        ...
        mean_cluster_diff = (df["n_clusters_found"] - df["n_clusters_true"]).abs().mean()
        print(f"\n{name} Evolution:")
    ...
    fig_moderate = plot_temporal_results(df_moderate, "Moderate Evolution (mutation_rate=0.20)")
    fig_slow = plot_temporal_results(df_slow, "Slow Evolution (mutation_rate=0.10)")
    
    # Save plots
    fig_fast.savefig("results/temporal_benchmark_fast.png", dpi=150, bbox_inches="tight")
    fig_moderate.savefig("results/temporal_benchmark_moderate.png", dpi=150, bbox_inches="tight")
    fig_slow.savefig("results/temporal_benchmark_slow.png", dpi=150, bbox_inches="tight")
    
    print("\nPlots saved to results/temporal_benchmark_*.png")
    plt.show()
