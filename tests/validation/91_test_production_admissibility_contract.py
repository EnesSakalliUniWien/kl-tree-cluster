...
def test_production_admissibility_contract_summarizes_decisions() -> None:
    ...
    assert (
        decisions["data_independent_gate_transfer_diagnostic_contract"]
        == "diagnostic_only"
    )
    ...
    assert (
        decisions["data_independent_gate_fail_closed_contract"]
        == "fail_closed_undefined"
    )
    assert (
        decisions["data_independent_gate_transfer_fail_closed_contract"]
        == "fail_closed_undefined"
    )

    blocked = summary[
        summary["contract_id"].eq("fail_closed_contract")
    ].iloc[0]
    assert blocked["blocking_component_ids"] == "external_tail"
    equation_blocked = summary[
        summary["contract_id"].eq("equation_fail_closed_contract")
    ].iloc[0]
