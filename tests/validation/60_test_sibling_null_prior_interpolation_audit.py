...
import pandas as pd
import pytest
from benchmarks.diagnostics.calibration.sibling_null_prior_interpolation_audit import (
...
def test_interpolation_audit_smoke_writes_diagnostic_outputs() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert tables.case_summary.iloc[0]["diagnostic_role"].endswith(
            "not_calibration"
        )
        assert (
            tables.case_summary.iloc[0]["n_selected_nonnull_positive_weight_records"]
            >= 0
        )
