...
def test_deformed_mp_edge_rows_compute_adjusted_root_excess() -> None:
    ...
    row = rows.iloc[0]
    assert row["deformed_mp_edge_status"] == (
        "deformed_mp_edge_computed_diagnostic_only"
    )
    ...
    assert row["production_inference_status"] == (
        "fail_closed_deformed_mp_edge_diagnostic_only"
    )
...
def test_deformed_mp_edge_rows_respect_exact_tail_support() -> None:
    ...
    assert rows.iloc[0]["deformed_mp_edge_status"] == (
        "exact_tail_support_h_u_optional"
    )
...
def test_deformed_mp_edge_rows_mark_missing_bulk_inputs() -> None:
    ...
    assert rows.iloc[0]["deformed_mp_edge_status"] == (
        "deformed_mp_edge_missing_inputs"
    )
