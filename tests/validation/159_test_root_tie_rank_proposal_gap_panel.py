...
import pandas as pd
from benchmarks.diagnostics.calibration import root_tie_rank_proposal_gap_panel as panel
...
def test_gap_summary_reports_family_failure_modes() -> None:
    rows = panel.build_root_tie_rank_proposal_gap_rows(_combined_rows())
    summary = panel.summarize_root_tie_rank_proposal_gap_rows(rows).set_index(
        "proposal_family"
    )

    assert summary.loc["exact_family", "summary_status"] == (
        "proposal_hits_observed_target_strata"
    )
