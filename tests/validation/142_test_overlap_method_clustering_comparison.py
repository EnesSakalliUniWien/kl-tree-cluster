...
import numpy as np
import pandas as pd
from benchmarks.diagnostics.calibration.overlap_method_clustering_comparison import (
    DEFAULT_LEFT_METHOD,
...
def _write_checkpoint(
    ...
    *,
    ...
    stem = (
        f"case={case_id}__role={role}__method={method_id}"
        f"__replicate={replicate:04d}"
    )
    genes = pd.DataFrame(
        {
            "case_id": case_id,
...
def test_load_assignment_record_computes_null_truth_metrics(tmp_path: Path) -> None:
    ...
    record = load_assignment_record(
        root
        / (
            "case=overlap_null__role=null"
            f"__method={DEFAULT_LEFT_METHOD}__replicate=0000.genes.csv"
        )
    )
