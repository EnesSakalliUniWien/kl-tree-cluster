...
import numpy as np
import pandas as pd
from benchmarks.validation.tree.brancharchitect_tree_comparison import (
    BUNDLED_BRANCHARCHITECT_PATH,
...
def test_relocated_comparison_resolves_bundled_adapter_path() -> None:
    expected = Path(__file__).resolve().parents[3] / "vendor" / "BranchArchitect"
    assert BUNDLED_BRANCHARCHITECT_PATH == expected
...
def test_writer_emits_artifacts_without_brancharchitect(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(
        "benchmarks.validation.tree.brancharchitect_tree_comparison.BUNDLED_BRANCHARCHITECT_PATH",
