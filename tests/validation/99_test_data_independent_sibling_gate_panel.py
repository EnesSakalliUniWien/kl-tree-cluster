...
def test_coordinate_gate_bh_matches_statsmodels_adjusted_minimum() -> None:
    ...
    assert p_value == pytest.approx(
        float(np.min(multipletests(coordinate_p, method="fdr_bh")[1]))
    )
...
def test_summary_marks_null_candidate_and_inflated_statuses() -> None:
    ...
    assert (
        candidate.iloc[0]["data_independent_gate_status"]
        == "data_independent_gate_null_candidate"
    )
    assert (
        inflated.iloc[0]["data_independent_gate_status"]
...
def test_summary_marks_signal_retained_and_weak_statuses() -> None:
    ...
    assert (
        retained.iloc[0]["data_independent_gate_status"]
        == "data_independent_gate_signal_retained"
    )
    assert (
        weak.iloc[0]["data_independent_gate_status"]
