...
def test_spectral_lift_target_rows_find_replay_candidate_and_lift_gap() -> None:
    ...
    assert rows.loc["setting_a", "spectral_lift_multiplier_required"] == pytest.approx(
        4.0
    )
...
def test_spectral_lift_summary_reports_setting_statuses() -> None:
    ...
    assert summary.loc["setting_a", "summary_status"] == (
        "setting_requires_more_spectral_lift"
    )
...
def test_conditioned_coherent_settings_use_target_geometry_without_spectral_target() -> None:
    ...
    assert by_target["hard"]["proposal_spike_feature_fraction"] < by_target["easy"][
        "proposal_spike_feature_fraction"
    ]
    assert by_target["hard"]["proposal_spike_delta"] > by_target["easy"][
