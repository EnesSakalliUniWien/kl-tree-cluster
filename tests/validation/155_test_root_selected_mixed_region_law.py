...
import pandas as pd
import pytest
from benchmarks.diagnostics.calibration import root_selected_mixed_region_law as panel
    ...
    return pd.DataFrame.from_records(
        [
            ...
            },
            {
                "case_id": "case_mixed",
                "root_selected_region_law_status": (
                    "mixed_smooth_and_nonsmooth_geometry"
                ),
...
def test_mixed_law_classifies_discrete_root_and_bandwidth_reopen() -> None:
    ...
    assert row["root_calibration_status"] == (
        "blocked_until_discrete_tie_rank_null_calibrated"
    )
    assert row["root_continuous_margin_status"] == (
        "active_or_numerically_tied_margin"
    )
    ...
    assert row["root_bandwidth_locality_status"] == (
        "bandwidth_reopens_without_root_law_support"
    )
    assert row["root_mixed_law_inference_status"] == (
        "fail_closed_discrete_root_law_missing_bandwidth_descriptive"
    )
    assert row["root_rank_fraction_edge_margin_product"] == pytest.approx(
        0.5 * math.log1p(15.0)
    )
...
def test_mixed_law_handles_smooth_and_mixed_regions() -> None:
    ...
    assert smooth["root_calibration_status"] == (
        "smooth_first_order_candidate_diagnostic_only"
    )
    ...
    assert mixed["root_mixed_region_component"] == (
        "mixed_smooth_and_discrete_region"
    )
    assert mixed["root_calibration_status"] == (
        "blocked_until_discrete_tie_rank_null_calibrated"
    )
    assert mixed["root_bandwidth_locality_status"] == (
        "bandwidth_does_not_reopen_root_rows"
    )
...
def test_mixed_law_counts_direct_measurable_root_frontier_rows() -> None:
    ...
    assert smooth["root_bandwidth_locality_status"] == (
        "bandwidth_does_not_reopen_root_rows"
    )
