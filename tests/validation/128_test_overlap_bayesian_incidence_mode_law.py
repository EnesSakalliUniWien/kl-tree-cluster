...
import pandas as pd
from benchmarks.diagnostics.calibration.overlap_bayesian_incidence_mode_law import (
...
def test_incidence_mode_law_uses_metric_family_alignment_when_available() -> None:
    ...
    assert rows["bayesian_incidence_mode_status"].iloc[0] == (
...
def test_incidence_mode_summary_reports_unidentified_emergent_mode() -> None:
    ...
    assert summary["diagnostic_status"].iloc[0] == (
        "context_negative_emergent_mode_not_identified"
    )
