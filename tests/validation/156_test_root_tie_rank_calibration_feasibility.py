...
def test_feasibility_rows_count_only_selected_null_support() -> None:
    ...
    assert signal["calibration_feasibility_status"] == (
    ...
    assert unlabeled["calibration_feasibility_status"] == (
...
def test_feasibility_strata_and_summary_report_missing_support() -> None:
    ...
    missing = strata[
        strata["stratum_feasibility_status"].eq("external_null_support_missing")
    ].iloc[0]
...
def test_unjoined_bandwidth_frontier_is_missing_not_no_reopen() -> None:
    ...
    assert "bandwidth=bandwidth_reopen_missing" in row[
