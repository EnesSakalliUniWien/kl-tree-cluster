...
import pandas as pd
from benchmarks.diagnostics.calibration.selected_tail_topology_refinement import (
...
def test_topology_refined_tail_laws_report_base_and_refinements() -> None:
    ...
    assert {"base", "topology", "spectral_alignment"} <= set(
        tail_laws["context_family"]
    )
    ...
    diagnostic_passed_refined = refined_rows[
        refined_rows["diagnostic_tail_law_contract_passed"]
    ]
    assert not diagnostic_passed_refined.empty
    assert diagnostic_passed_refined["production_tail_law_failure_reasons"].str.contains(
        DATA_ADAPTIVE_PRODUCTION_BLOCK_REASON,
        regex=False,
    ).all()
    ...
    assert (
        summary["n_precision_pass_contexts"].sum()
        == summary["n_descriptive_contexts"].sum()
    )
    assert (
        summary.loc[
