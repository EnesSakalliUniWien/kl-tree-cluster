...
def test_proxy_rows_derive_balances_and_scores() -> None:
    ...
    assert (
        float(recovery["balanced_recovery_proxy_score"])
        > float(fragment["balanced_recovery_proxy_score"])
    )
    assert (
        float(fragment["fragment_risk_proxy_score"])
        > float(recovery["fragment_risk_proxy_score"])
    )
