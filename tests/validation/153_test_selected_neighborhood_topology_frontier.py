...
def test_topology_frontier_joins_root_selected_region_status() -> None:
    ...
    assert root["root_selected_law_status"] == (
        "discrete_tie_cell_geometry_required"
    )
...
def test_nonroot_frontier_requires_current_floor_and_spectral_support() -> None:
    ...
    assert by_node["near"]["hybrid_strict_status"] == (
        "nonroot_topology_below_current_floor"
    )
...
def test_summary_and_sweep_expose_bandwidth_reopening_risk() -> None:
    ...
    signal_root_sweep = sweep[
        sweep["data_role"].eq("signal") & sweep["threshold"].eq(0.20)
    ].iloc[0]
