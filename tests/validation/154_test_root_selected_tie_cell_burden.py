...
import pandas as pd
import pytest
from benchmarks.diagnostics.calibration import root_selected_tie_cell_burden as panel
...
def test_tie_cell_burden_sums_log_multiplicity() -> None:
    ...
    assert case_a["root_tie_cell_status"] == (
...
def test_tie_cell_relationships_are_reported() -> None:
    ...
    evaluated = relationships[
        relationships["covariate"].eq("root_tie_cell_log_burden")
    ].iloc[0]
