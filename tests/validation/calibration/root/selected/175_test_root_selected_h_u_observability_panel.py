...
def test_h_u_observability_blocks_compressed_root_spectrum() -> None:
    ...
    assert row["h_u_observability_status"] == ("h_u_estimation_blocked_missing_capture")
...
def test_h_u_observability_marks_missing_support_before_estimation() -> None:
    ...
    assert rows.iloc[0]["h_u_observability_status"] == ("support_missing_before_h_u_estimation")
...
def test_h_u_observability_defers_to_exact_tail_support() -> None:
    ...
    assert rows.iloc[0]["h_u_observability_status"] == ("exact_tail_support_available_h_u_optional")
