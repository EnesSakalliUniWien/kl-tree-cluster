...
def test_neighborhood_join_audit_labels_missing_replay_and_join_states(
    ...
    assert by_case.loc["observed", "bandwidth_join_status"] == ("bandwidth_measured_available")
    assert by_case.loc["generated_missing_frontier", "bandwidth_join_status"] == (
        "generated_matrix_available_topology_frontier_absent_after_mixed_frontier_not_joined"
    )
    assert (
        by_case.loc[
...
def test_neighborhood_join_audit_counts_direct_measurable_root_frontier(
    ...
    assert row["bandwidth_join_status"] == ("topology_frontier_rows_present_no_reopen_not_joined")
...
def test_neighborhood_join_summary_reports_next_required_work(tmp_path: Path) -> None:
    ...
    assert summary.iloc[0]["summary_status"] == ("generated_topology_frontier_replay_needed")
