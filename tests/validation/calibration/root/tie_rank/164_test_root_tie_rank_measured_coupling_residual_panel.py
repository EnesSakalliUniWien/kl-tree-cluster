...
def _row(
    *,
    ...
    return {
        ...
        "generated_measured_neighborhood_coupling": target_bottleneck * measured_ratio,
...
def test_residual_rows_localize_spectral_and_action_edge_deficits() -> None:
    ...
    residual = panel.build_measured_coupling_residual_rows(rows).set_index("target_case_id")

    assert residual.loc["spectral_hard", "dominant_residual_axis"] == ("spectral_excess")
    assert residual.loc["spectral_hard", "next_mathematical_step"] == (
        "derive_selected_spectral_excess_given_high_action_edge_tie_rank"
    )
    assert residual.loc["action_hard", "dominant_residual_axis"] == ("action_edge_bottleneck")
