...
import pandas as pd
import pytest
from benchmarks.diagnostics.calibration.root.center import (
...
def test_tree_geometry_parser_requires_builder_rooting_distance_linkage() -> None:
    geometries = panel.parse_tree_geometries(
        "linkage:linkage_root:hamming:average,neighbor_joining:mad:jaccard:average"
    )
        ...
        [
            {
                ...
                "hard_negative_control_status": ("hard_negative_control_blocked_root_unstable"),
                "root_stability_subsample_mean_ari": 0.001,
...
def test_hard_negative_summary_counts_rootless_truth_aligned_geometries() -> None:
    ...
    assert summary.iloc[0]["max_best_unrooted_edge_cut_truth_ari"] == pytest.approx(0.72)
