...
def test_sibling_inflation_diagnostic_exposes_blocking_inflation_threshold() -> None:
    ...
    assert contributors.iloc[0]["local_weight_share"] > contributors.iloc[1]["local_weight_share"]
...
def test_sibling_inflation_diagnostic_reports_calibration_variants() -> None:
    ...
    assert target["current_empirical_inflation_factor"] == target["leave_one_out_inflation_factor"]
    assert target["leave_one_out_inflation_factor"] == target["strict_null_like_inflation_factor"]
    assert (
        target["edge_blocked_or_null_like_inflation_factor"]
        == target["leave_one_out_inflation_factor"]
    )
