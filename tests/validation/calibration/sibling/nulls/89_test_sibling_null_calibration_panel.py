...
import pandas as pd
import pytest
from benchmarks.diagnostics.calibration.sibling.nulls.sibling_null_calibration_panel import (
    STUDY_ROLE,
...
def test_sibling_null_panel_scores_context_roles() -> None:
    ...
    selected = summary[summary["sibling_context_role"].eq("selected_nonnull_only")].iloc[0]
    external = summary[summary["sibling_context_role"].eq("external_selected_tail_context")].iloc[0]
    ...
    assert selected["sibling_calibration_status"] == ("selected_nonnull_retention_descriptive")
