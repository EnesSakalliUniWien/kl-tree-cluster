...
import benchmarks.diagnostics.calibration.sibling.gates.data_independent_sibling_gate_traversal_panel as traversal_panel
import pandas as pd
...
def test_summarize_traversal_rows_marks_null_and_signal_statuses() -> None:
    ...
    assert float(summary.loc[summary["data_role"].eq("signal"), "root_split_rate"].iloc[0]) == 1.0
...
def test_summarize_traversal_transfer_marks_transfer_candidate() -> None:
    ...
    assert transfer.iloc[0]["additional_zero_false_split_null_replicates_per_case"] == 57
...
def test_build_traversal_production_components_keeps_candidates_diagnostic_only() -> None:
    ...
    assert (
        decisions["data_independent_traversal_transfer:binary_template:coordinate_bh:penalty=500"]
        == "fail_closed_undefined"
    )
    assert (
        decisions[
            "data_independent_traversal_transfer:categorical_multinomial:coordinate_bh:penalty=500"
        ]
        == "fail_closed_undefined"
    )
...
def test_run_data_independent_sibling_gate_traversal_panel_writes_outputs(
    ...
    assert (
        pd.to_numeric(rows["root_observed_p_value"], errors="coerce")
        .between(
            0.0,
            1.0,
        )
...
def test_run_traversal_panel_computes_root_selective_permutation_lazily(
    ...
    def counted_root_selective(*_args, bootstrap_replicates: int, **_kwargs):
        ...
        return {
            ...
            "root_selective_p_value": pd.NA if int(bootstrap_replicates) <= 0 else 0.5,
            "root_selective_null_min_p_value": pd.NA if int(bootstrap_replicates) <= 0 else 0.5,
            "root_selective_null_q05_p_value": pd.NA if int(bootstrap_replicates) <= 0 else 0.5,
        }
