...
import benchmarks.diagnostics.calibration.sibling.gates.fixed_sibling_gate_profile_validation as profile_validation
import pandas as pd
from benchmarks.diagnostics.calibration.sibling.gates.fixed_sibling_gate_profile_validation import (
...
def test_profile_transfer_components_fail_closed_when_coverage_is_missing() -> None:
    ...
    assert transfer.iloc[0]["profile_transfer_status"] == ("fixed_profile_insufficient_coverage")
    assert "fixed_profile_insufficient_coverage" in set(components["component_status"])
...
def test_profile_transfer_summary_reports_null_support_sizing() -> None:
    ...
    assert transfer.iloc[0]["required_zero_false_split_null_replicates_per_case"] == 73
    assert transfer.iloc[0]["additional_zero_false_split_null_replicates_per_case"] == 71
    assert transfer.iloc[0]["max_observed_null_false_split_count"] == 0
    assert transfer.iloc[0]["max_required_null_replicates_given_observed_false_splits"] == 73
    assert (
        transfer.iloc[0]["max_additional_zero_false_null_replicates_given_observed_false_splits"]
        == 71
    )
...
def test_profile_transfer_summary_sizes_observed_false_split_support() -> None:
    ...
    assert transfer.iloc[0]["max_required_null_replicates_given_observed_false_splits"] == 110
    assert (
        transfer.iloc[0]["max_additional_zero_false_null_replicates_given_observed_false_splits"]
        == 37
    )
...
def test_run_fixed_profile_validation_writes_outputs_and_evidence(
    ...
    assert summary["profile_validation_status"].isin({"fixed_profile_null_candidate"}).all()
    assert "sibling_gate_profile" in evidence["constants"]
    assert "adaptive_projection_avoidance_check" in evidence["constants"]["sibling_gate_profile"]
    assert evidence["constants"]["sibling_gate_profile"]["profile_grid"] == [
        "fixed_global_guarded_v1"
    ]
    assert (
        evidence["constants"]["sibling_gate_profile"]["root_selective_permutation_guard"]["status"]
        == "enabled"
    )
    ...
    assert evidence["constants"]["root_selective_permutation_guard_scope"]["scope_grid"] == ["root"]
    null_confidence = evidence["constants"]["sibling_gate_profile"]["null_false_split_confidence"]
    assert "required_zero_false_split_null_replicates_per_case" in null_confidence[0]
    assert "max_required_null_replicates_given_observed_false_splits" in null_confidence[0]
...
def test_evidence_uses_profile_owned_root_selective_guard_constants(tmp_path) -> None:
    ...
    summary = pd.DataFrame.from_records([{"adaptive_projection_avoided_rate": 1.0}])
    ...
    assert guard_evidence["by_profile"]["fixed_coordinate_selective_root_v1"]["replicates"] == 99
    assert guard_evidence["by_profile"]["fixed_coordinate_selective_root_v1"]["alpha"] == 0.01
    assert evidence["constants"]["root_selective_permutation_guard_replicates"][
        "permutation_replicate_grid"
    ] == [99]
    assert evidence["constants"]["root_selective_permutation_guard_alpha"]["alpha_grid"] == [0.01]
    assert evidence["constants"]["root_selective_permutation_guard_scope"]["scope_grid"] == ["root"]
...
def test_evidence_reports_global_passthrough_scope(tmp_path) -> None:
    ...
    assert (
        guard_evidence["by_profile"]["fixed_coordinate_global_passthrough_v1"]["scope"]
        == "global_sibling_min_passthrough_descendant"
    )
    assert evidence["constants"]["root_selective_permutation_guard_scope"]["scope_grid"] == [
        "global_sibling_min_passthrough_descendant"
    ]
