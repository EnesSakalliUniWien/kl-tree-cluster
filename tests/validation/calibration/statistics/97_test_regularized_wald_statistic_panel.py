...
import pandas as pd
import pytest
from benchmarks.diagnostics.calibration.statistics.regularized_wald_statistic_panel import (
...
def test_run_regularized_wald_statistic_panel_writes_outputs(tmp_path: Path) -> None:
    ...
    assert {"plugin", "jeffreys", "dirichlet_1", "root_shrink_0.1"} <= set(rows["smoothing_rule"])
    assert not summary.empty
    assert (
        production_summary["production_decision"]
        .isin({"diagnostic_only", "fail_closed_undefined", "production_admissible"})
        .all()
    )
