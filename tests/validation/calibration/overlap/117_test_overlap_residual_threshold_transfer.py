...
import pandas as pd
from benchmarks.diagnostics.calibration.overlap.overlap_residual_threshold_transfer import (
...
def test_transfer_rows_evaluate_leave_one_splits() -> None:
    ...
    null_holdout = rows[
        rows["threshold_name"].eq("null_evidence_threshold") & rows["holdout_value"].eq("a")
    ].iloc[0]
...
def test_transfer_summary_reports_retention_and_leakage() -> None:
    ...
    null_summary = summary[summary["threshold_name"].eq("null_evidence_threshold")].iloc[0]
