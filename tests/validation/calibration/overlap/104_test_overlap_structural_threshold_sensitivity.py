...
import pandas as pd
from benchmarks.diagnostics.calibration.overlap.overlap_structural_threshold_sensitivity import (
...
def test_recommendations_score_threshold_tradeoff() -> None:
    ...
    strict = recommendations[recommendations["homogeneity_gain_threshold"].eq(0.02)].iloc[0]
    permissive = recommendations[recommendations["homogeneity_gain_threshold"].eq(0.0)].iloc[0]
    ...
    assert permissive["threshold_stability_status"] == "threshold_unstable_null_false_accepts"
