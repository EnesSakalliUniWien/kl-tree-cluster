...
import pandas as pd
from benchmarks.diagnostics.calibration.overlap.overlap_threshold_stability_contract import (
...
def test_threshold_stability_contract_marks_nontransferable_and_law_required() -> None:
    ...
    by_stage = dict(zip(rows["stage_name"], rows["threshold_stability_status"], strict=True))
    assert by_stage["continuous_structural_stable_accept"] == ("stable_reporting_candidate")
    ...
    assert by_stage["residual_null_evidence_only_unresolved"] == ("selected_family_law_required")
