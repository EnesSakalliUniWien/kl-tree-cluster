...
def test_conditional_status_requires_structural_neighborhood_support() -> None:
    ...
    status_by_case = dict(zip(rows["case_id"], rows["conditional_bayesian_status"], strict=True))

    assert status_by_case["coherent_case"] == "conditional_coherent_candidate"
    assert status_by_case["fragment_case"] == "p_value_extreme_structurally_incoherent"
