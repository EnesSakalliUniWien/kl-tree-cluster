...
import pandas as pd
from benchmarks.diagnostics.calibration.overlap.overlap_structural_context_thresholds import (
...
def test_context_threshold_summary_identifies_context_specific_cutoffs() -> None:
    ...
    shallow = summary[
        summary["context_axis"].eq("depth_bin") & summary["context_bin"].eq("shallow_1_2")
    ].iloc[0]
    deep = summary[
        summary["context_axis"].eq("depth_bin") & summary["context_bin"].eq("deep_3_plus")
    ].iloc[0]
