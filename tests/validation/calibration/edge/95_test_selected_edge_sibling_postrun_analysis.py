...
import pandas as pd
from benchmarks.diagnostics.calibration.edge.selected_edge_sibling_postrun_analysis import (
...
def test_run_selected_edge_sibling_postrun_analysis_writes_outputs(tmp_path: Path) -> None:
    ...
    assert (
        ...
        .isin({"fail_closed_undefined", "diagnostic_only", "production_admissible"})
        .all()
    )
