...
def test_retained_pass_through_rows_define_signal_and_controls() -> None:
    ...
    signal = rows.loc[rows["likelihood_class"].eq("signal_retained_pass_through")].iloc[0]
    control = rows.loc[rows["likelihood_class"].eq("selected_null_pass_through_control")].iloc[0]
...
def test_retained_pass_through_matching_and_identifiability_fail_closed() -> None:
    ...
    assert summary["production_action"].iloc[0] == ("fail_closed_until_likelihood_identifiable")
