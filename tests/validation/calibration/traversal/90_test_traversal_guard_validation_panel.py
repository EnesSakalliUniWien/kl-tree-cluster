...
import pandas as pd
from benchmarks.diagnostics.calibration.traversal.traversal_guard_validation_panel import (
    STUDY_ROLE,
        ...
        rows.append(
            {
                "case_id": "signal",
                "parent_id": f"s{index}",
                "traversal_context_role": "mixed_signal" if index % 2 == 0 else "true_signal",
...
def test_traversal_guard_panel_scores_candidate_guard() -> None:
    ...
    candidate = summary[summary["guard_id"].eq("action_ge_0.9__angle_ge_75__ind_ge_0.85")].iloc[0]
...
def test_traversal_guard_panel_keeps_low_precision_guard_diagnostic_only() -> None:
    ...
    broad = summary[summary["guard_id"].eq("action_ge_0.9__angle_ge_75__ind_ge_0.85")].iloc[0]
