...
def test_feature_geometry_separates_when_balance_product_leaks() -> None:
    rows = build_branch_recovery_conditioning_rows(build_focused_branch_recovery_fixture_rows())
    metrics = summarize_branch_recovery_conditioning_metrics(rows)
    by_metric = {row["metric"]: row for _, row in metrics.iterrows()}

    assert (
        by_metric["feature_branch_geometry_score"]["zero_negative_status"]
        == "zero_negative_separates_all_branch_recovery"
    )
    assert (
        by_metric["feature_homogeneity_gain_min"]["zero_negative_status"]
        == "zero_negative_separates_all_branch_recovery"
    )
    assert (
        by_metric["branch_recovery_oracle_score"]["zero_negative_status"]
        == "zero_negative_separates_all_branch_recovery"
    )
...
def test_summary_reports_observable_fixture_but_keeps_production_fail_closed() -> None:
    ...
    assert summary["production_action"].iloc[0] == ("fail_closed_until_branch_law_validated")
...
def test_summary_reports_missing_full_branch_support_for_real_like_rows() -> None:
    ...
    assert summary["diagnostic_status"].iloc[0] == ("full_branch_recovery_support_missing")
...
def test_feature_geometry_builder_uses_paths_without_truth_predictors() -> None:
    ...
    child_by_sample = {
        sample_id: f"child_{int(label) % 2}" for sample_id, label in label_values.items()
    }
