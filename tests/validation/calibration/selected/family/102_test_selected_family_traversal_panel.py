...
import benchmarks.diagnostics.calibration.selected.family.selected_family_traversal_panel as selected_panel
...
def test_multiscale_node_decisions_distinguish_guard_and_traversal_classes() -> None:
    ...
    assert not bool(rows.loc[rows["node_id"].eq("root"), "topology_has_incoming_edge"].iloc[0])
    assert bool(rows.loc[rows["node_id"].eq("B"), "topology_has_outgoing_test"].iloc[0])
    assert (
        rows.loc[
            rows["node_id"].eq("B1"),
            "conditional_topology_status",
        ].iloc[0]
        == "leaf_no_outgoing_test_fail_closed"
    )
    assert (
        rows.loc[
            rows["node_id"].eq("B"),
...
def test_production_components_remain_diagnostic_or_fail_closed() -> None:
    ...
    assert "fixed_profile_transfer_candidate" in set(components["component_status"])
...
def test_run_selected_family_traversal_panel_writes_outputs(tmp_path) -> None:
    ...
    assert (
        production["production_decision"].isin({"diagnostic_only", "fail_closed_undefined"}).all()
    )
