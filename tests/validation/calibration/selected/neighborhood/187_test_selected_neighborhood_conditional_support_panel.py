...
def test_hard_negative_root_invalid_blocks_local_support() -> None:
    rows = pd.DataFrame.from_records([_row(case_id="overlap_extreme_4c", node_id="candidate")])
    ...
    assert candidate["dominant_blocker"] == ("root_validity_failed_hard_negative_control")
...
def test_selected_null_support_is_reported_as_leak_diagnostic() -> None:
    ...
    assert candidate["method_action"] == ("selected_null_conditional_support_leak_diagnostic")
...
def test_direct_split_is_not_counted_as_neighborhood_support() -> None:
    ...
    assert candidate["method_action"] == ("direct_split_already_measured_not_neighborhood_rescue")
    assert not bool(candidate["conditional_support_pass"])
