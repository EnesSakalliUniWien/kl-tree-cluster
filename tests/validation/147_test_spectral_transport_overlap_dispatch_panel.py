...
def test_pairwise_rows_label_spectral_block_less_fragmented_not_worse() -> None:
    ...
    computed = {
        ("toy_overlap", panel.BASELINE_METHOD): SimpleNamespace(
            labels=np.array([0, 0, 1, 2])
        ),
        ("toy_overlap", panel.CANDIDATE_METHOD): SimpleNamespace(
...
def test_run_spectral_transport_overlap_dispatch_panel_writes_outputs(
    ...
    assert pairwise.iloc[0]["guard_effect_class"] == (
        "spectral_block_less_fragmented_not_worse"
    )
