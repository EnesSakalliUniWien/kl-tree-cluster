...
def _target_row(
    *,
    ...
    return {
        ...
        "target_moment_vector_json": json.dumps(
            {"T": 0.8, "A": 6.0, "E": 6.0, "S_Hu": s_h_u}
        ),
    }
...
def test_summary_counts_required_axes_outside_hull() -> None:
    rows = _rows_for(s_h_u=0.5)
    summary = panel.summarize_root_selected_conditional_tilt_feasibility_rows(
        rows
    ).iloc[0]
