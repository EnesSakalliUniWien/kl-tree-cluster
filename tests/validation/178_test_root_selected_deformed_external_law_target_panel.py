...
def test_external_law_target_records_conditional_tilt_moments() -> None:
    ...
    assert row["conditional_law_family"] == (
        "selected_root_conditional_exponential_tilt"
    )
...
def test_external_law_target_summary_counts_existing_support() -> None:
    ...
    summary = panel.summarize_root_selected_deformed_external_law_target_rows(rows).iloc[
        0
    ]
