...
from benchmarks.diagnostics.calibration.overlap_selected_pass_through_fixture_miner import (
...
def test_structural_topology_context_uses_incoming_and_outgoing_tree_balance() -> None:
    ...
    assert pass_row["structural_topology_context_status"] == (
        "structural_topology_observed"
    )
...
def test_selected_pass_through_fixture_miner_writes_outputs(tmp_path) -> None:
    ...
    assert summary["production_action"].iloc[0] == (
        "fail_closed_until_fixture_support_observed"
    )
