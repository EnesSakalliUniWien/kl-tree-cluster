...
import pandas as pd
from benchmarks.diagnostics.calibration.selected_neighborhood_distribution_panel import (
...
def test_selected_neighborhood_rows_classify_traversal_stop_reasons() -> None:
    ...
    assert by_node["pass"]["traversal_stop_reason"] == (
...
def test_selected_neighborhood_distribution_summaries_are_robust() -> None:
    ...
    split_balance = summary.loc[
        summary["metric"].eq("balance_product")
        & summary["traversal_state"].eq("split")
    ].iloc[0]
...
def test_selected_neighborhood_coverage_summarizes_evidence_by_state() -> None:
    ...
    pass_through = coverage.loc[
        coverage["traversal_state"].eq("pass_through")
    ].iloc[0]
...
def test_selected_neighborhood_case_coverage_preserves_case_id() -> None:
    ...
    split = case_coverage.loc[
        case_coverage["case_id"].eq("case")
        & case_coverage["traversal_state"].eq("split")
    ].iloc[0]
...
def test_selected_neighborhood_candidate_method_contrast_rows_explain_candidates() -> None:
    ...
    assert by_node["pass"]["candidate_contrast_status"] == (
        "candidate_decision_diverges"
    )
    assert "left_pass_through" in by_node["pass"]["candidate_reason"]
    assert by_node["guarded"]["candidate_contrast_status"] == (
        "candidate_decision_agrees"
    )
...
def test_selected_neighborhood_candidate_rows_count_descendant_outcomes() -> None:
    ...
    profile_contract = summarize_selected_neighborhood_profile_config_contract(
        candidate_rows
    )
    ...
    retention_row = retention.loc[
        retention["stop_rule_pattern"].eq(
    ...
    assert target["evidence_gap"] == (
        "missing_topology_evidence_for_downstream_splits"
    )
    assert stop_rule["stop_rule_pattern"] == (
        "left_pass_through_downstream_split_right_stops"
    )
    ...
    signal_gap = gaps.loc[
        gaps["retention_evidence_status"].eq(
            "retention_evidence_signal_unresolved_no_topology"
        )
    ].iloc[0]
    ...
    assert signal_gap["production_action"] == (
    ...
    assert by_field["root_selective_permutation_guard_scope"][
        "profile_contract_status"
    ] == "refined_selected_family_guard_difference"
    readiness_by_scope = {
        row["readiness_scope"]: row for _, row in readiness.iterrows()
    }
    assert readiness_by_scope["selected_null_pass_through_control"][
        "readiness_status"
    ] == "ready_as_fail_closed_guard_candidate"
    assert readiness_by_scope["signal_pass_through_retention"][
        "readiness_status"
...
def test_selected_neighborhood_distribution_panel_writes_outputs(tmp_path) -> None:
    ...
    candidate_method_contrast = pd.read_csv(
        outputs["candidate_method_contrast_summary"]
    )
    candidate_method_contrast_rows = pd.read_csv(
        outputs["candidate_method_contrast_rows"]
    )
    candidate_ambiguity = pd.read_csv(outputs["candidate_ambiguity_summary"])
    candidate_local_features = pd.read_csv(
        outputs["candidate_local_feature_summary"]
    )
