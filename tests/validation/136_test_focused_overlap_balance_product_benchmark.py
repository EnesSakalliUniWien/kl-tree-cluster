...
import pandas as pd
from benchmarks.diagnostics.calibration.focused_overlap_balance_product_benchmark import (
...
def test_focused_overlap_benchmark_writes_outputs(tmp_path) -> None:
    ...
    assert metric_summary.loc[
        metric_summary["metric"].eq("balance_product"),
        "zero_negative_status",
    ].iloc[0] == "zero_negative_separates_all_truth"
