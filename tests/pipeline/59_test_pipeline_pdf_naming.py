...
from benchmarks.shared.util.pdf.session import resolve_pdf_output_path
...
def test_resolve_pdf_output_path_defaults_to_timestamped_name_under_plots_root():
    ...
    out = resolve_pdf_output_path(None, plots_root=plots_root, started_at=started_at)
...
def test_resolve_pdf_output_path_adds_pdf_suffix_when_missing():
    plots_root = Path("/tmp/results/plots")
    started_at = datetime(2025, 1, 2, 3, 4, 5, tzinfo=timezone.utc)
    out = resolve_pdf_output_path("custom_name", plots_root=plots_root, started_at=started_at)
