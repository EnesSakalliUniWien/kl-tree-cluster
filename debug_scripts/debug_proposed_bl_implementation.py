"""Debug script showing proposed branch length integration implementation.

This file contains the proposed changes to:
1. edge_significance.py - Child-parent test with normalized BL
2. pooled_variance.py - Sibling test with normalized BL

Key formula:
    BL_normalized = 1 + BL / mean(BL)

This ensures:
- BL_normalized ≥ 1 (no variance shrinkage)
- Mean(BL_normalized) = 2 (moderate variance inflation)
- Longer branches → larger variance → smaller z → harder to split
- Shorter branches → smaller variance inflation → easier to split
"""
