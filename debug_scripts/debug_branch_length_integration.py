"""Debug script to analyze branch length integration options.

Investigates:
1. Current branch length distribution (from Hamming distance linkage)
2. How to normalize branch lengths for Felsenstein PIC
3. Effect of different normalization strategies on variance

Key insight from Felsenstein (1985):
- For independent contrasts, Var(contrast) = σ² × (BL_left + BL_right)
- Longer branches → more expected divergence → larger variance → smaller z
- Our BL are ~0.1-0.4, so multiplying by them makes variance SMALLER (wrong!)

Solution options:
1. Normalize BL by tree height so BL ∈ [0, 1] with height = 1
2. Normalize BL by mean so mean(BL) = 1
3. Use 1 + BL instead of BL (ensures multiplier ≥ 1)
4. Use BL / (1 - BL) to map [0,1) → [0, ∞)
"""
