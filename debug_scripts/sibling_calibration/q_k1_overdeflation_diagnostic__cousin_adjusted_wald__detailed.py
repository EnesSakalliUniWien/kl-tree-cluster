"""
Purpose: Detailed diagnostics for K=1 over-deflation in cousin-adjusted Wald calibration.
Inputs: Generated benchmark cases and sibling calibration outputs.
Outputs: Detailed console diagnostics across methods/cases.
Expected runtime: ~30-180 seconds.
How to run: python debug_scripts/sibling_calibration/q_k1_overdeflation_diagnostic__cousin_adjusted_wald__detailed.py
"""
...
def diagnose_adjusted_wald(tree, data_df):
    ...
    print(
        f"  {'Parent':<10} {'n_parent':>8} {'BL_sum':>8} {'T_raw':>8} {'k':>4} {'ĉ':>8} {'T_adj':>8} {'p_adj':>8} {'null?':>6}"
    )
    print(f"  {'-'*80}")
    ...
    if model.method == "regression" and model.beta is not None:
        ...
        if null_records:
            ...
            print(
                f"  {'Parent':<10} {'n_parent':>8} {'T_raw':>8} {'ĉ_reg':>8} {'ĉ_cap':>8} {'ĉ_noB2':>8} {'p_reg':>8} {'p_cap':>8} {'p_noB2':>8}"
            )
            print(f"  {'-'*90}")
...
def main():
    ...
    for i, case in enumerate(test_cases):
        ...
        if k_adj == 1 and k_wald > 1:
            print(
                f"  [{i+1}] {name}: TRUE_K={true_k}, wald=K{k_wald}, adj_wald=K{k_adj} *** K=1 COLLAPSE ***"
            )
