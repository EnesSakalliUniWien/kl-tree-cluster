...
def run_comparison():
    ...
    # For binary input, KL(P||Q) = p*log(p/q) + (1-p)*log((1-p)/(1-q))
    eps = 1e-10
    p = np.clip(np.array([P_prob]), eps, 1.0 - eps)
    q = np.clip(np.array([Q_prob]), eps, 1.0 - eps)
    kl_div = rel_entr(p, q) + rel_entr(1.0 - p, 1.0 - q)
    ...
    # KL
    kl_cat = np.sum(rel_entr(np.clip(P_cat, eps, 1.0), np.clip(Q_cat, eps, 1.0)), axis=-1)
    ...
    print(
        "This produces a slightly different statistic than the pure Pearson Chi-Square, but is monotonically related."
    )
