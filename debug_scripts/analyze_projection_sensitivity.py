"""Analysis: Improving projection test sensitivity with scipy.

Current issue: After fixing orthonormal projection, the test is too CONSERVATIVE.
- Case 55: Expected 12 clusters, finding 2
- Mean χ²/df ratio = 1.26 (close to 1.0 under H₀) but still under-splitting

Root causes:
1. Projection reduces d→k dimensions, losing signal spread across many features
2. Chi-square test may not be optimal for projected data
3. Need better sample-size correction for small subtrees

Potential scipy improvements:
1. Hotelling's T² with F-distribution (proper multivariate test)
2. scipy.stats.permutation_test for exact p-values
3. Better projection dimension selection
4. Use scipy.linalg.svd for PCA-like signal extraction
"""
