"""
Purpose: Compare hamming vs rogerstanimoto distance metrics for tree inference.
...
How to run: python debug_scripts/tree_construction/q_tree_metric_comparison__tree_construction__analysis.py
"""
...
def main():
    ...
    print(
        f"  rogerstanimoto: {df['rogerstanimoto_ari'].mean():.4f} (std={df['rogerstanimoto_ari'].std():.4f})"
    )
    ...
    if len(diff_cases) > 0:
        ...
        print(
            diff_cases[["case_name", "hamming_ari", "rogerstanimoto_ari", "ari_diff"]].to_string(
                index=False
            )
        )
    else:
