#!/usr/bin/env python
"""Debug why SBM hierarchical clustering finds only 1 cluster."""
...
def debug_sbm_tree():
    ...
    tree = PosetTree.from_linkage(
        Z=Z, distance_condensed=distance_condensed, sample_ids=sample_ids
    )

    print(f"Tree created with {len(list(tree.get_all_nodes()))} nodes")
...
""")
    ...
    try:
        tree.decompose_tree(alpha=0.99)  # Very permissive
        n_clusters = tree.get_n_clusters()
