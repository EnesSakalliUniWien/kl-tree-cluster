"""Diagnostic: Verify BH correction behavior under null hypothesis.

Generates null data (all leaves i.i.d.), builds linkage tree, runs the full
annotation pipeline, and inspects:

1. TreeBH family sizes (are they always 2 in a binary tree?)
2. TreeBH effective thresholds per family (how much correction is applied?)
3. Edge test rejection rate under null (should be ~5%)
4. Sibling test subset size vs total binary parents (selection bias?)
5. Sibling test rejection rate under null (should be ~5%)
6. Comparison: flat BH vs tree_bh vs level_wise on the same p-values
"""
