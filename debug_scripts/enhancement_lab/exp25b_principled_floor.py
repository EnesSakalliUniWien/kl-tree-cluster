"""Lab exp25b: Principled floor from JL concentration inequality.

Derivation (exp25_jl_divisor_theory.py):
  - JL lemma k = 4·ln(n) / (ε²/2 − ε³/3) ensures preservation of ALL n² pairwise distances.
  - The "4" decomposes as: 2 (union bound over n² pairs) × 2 (two-sided tail).
  - Gate 3 tests ONE sibling pair, one-sided (are they different?).
  - Principled single-pair detection threshold:
        k_floor = ⌈ln(1/α) / (ε²/2 − ε³/3)⌉
  - At α=0.01, ε=0.3:  k_floor = ⌈4.605/0.036⌉ = 128  (constant, n-independent)
  - At α=0.05, ε=0.3:  k_floor = ⌈2.996/0.036⌉ = 84

Configurations tested:
  1. production       — min_child spectral (current default)
  2. jl_floor_qrt     — max(spectral_min, JL/4)
  3. lam12_raw         — (λ₁+λ₂)/trace × JL (no floor)
  4. lam12_floor_8th   — max(lam12, JL/8)           [exp25 empirical winner]
  5. lam12_floor_p128  — max(lam12, 128)             [principled floor α=0.01]
  6. lam12_floor_p84   — max(lam12, 84)              [principled floor α=0.05]
  7. principled_only   — constant k=128 everywhere   [pure principled, no lam12]
"""
...
def principled_floor(alpha: float, eps: float) -> int:
    """k = ⌈ln(1/α) / (ε²/2 − ε³/3)⌉  —  single-pair detection threshold."""
    denom = eps**2 / 2 - eps**3 / 3
    return math.ceil(math.log(1.0 / alpha) / denom)


# Pre-compute for the two alpha levels
K_FLOOR_001 = principled_floor(0.01, config.PROJECTION_EPS)  # 128
K_FLOOR_005 = principled_floor(0.05, config.PROJECTION_EPS)  # 84
print(f"Principled floors: α=0.01 → k={K_FLOOR_001}, α=0.05 → k={K_FLOOR_005}")
...
def _compute_lam12_k(tree, parent, left, right, leaf_data):
    """Compute raw lam12_frac × JL for a parent node."""
    try:
...
def _derive_principled_only(tree, annotated_df):
    """Pure principled floor: constant k=K_FLOOR_001 for all nodes."""
    sibling_dims = {}
    for parent in tree.nodes:
        children = list(tree.successors(parent))
        if len(children) != 2:
            continue
        sibling_dims[parent] = K_FLOOR_001
    return sibling_dims if sibling_dims else None
...
CONFIGS = {
    ...
    "lam12_floor_p128": _make_lam12_strategy(_floor_p128),
    "lam12_floor_p84": _make_lam12_strategy(_floor_p84),
    "principled_only": _derive_principled_only,
}
...
if __name__ == "__main__":
    ...
    for i, case_name in enumerate(case_names, 1):
        ...
        p128 = res["lam12_floor_p128"]["ari"]
        print(f" {dt:5.1f}s  prod={prod:.3f}  p128={p128:.3f}")
    ...
    # ═════════════════════════════════════════════════════════════════════════
    # H2H: principled floor vs key baselines
    # ═════════════════════════════════════════════════════════════════════════

    focal = "lam12_floor_p128"
    for baseline in ["production", "lam12_raw", "lam12_floor_8th", "jl_floor_qrt"]:
        print(f"═══ H2H: {focal} vs {baseline} ═══\n")
        wins = losses = ties = 0
        win_lines = []
        loss_lines = []
        for case_name, res in all_results:
            ...
            if abs(delta) <= 0.005:
                ...
                win_lines.append(
                    ...
                    f"focal: K={r_focal['found_k']:>3} {r_focal['ari']:.3f}  "
    ...
    # Cases where p128 floor activates (different from raw lam12)
    print("\n  Cases where p128 floor activates (p128 ≠ raw):\n")
    n_activated = 0
    for case_name, res in all_results:
        r_raw = res["lam12_raw"]
        r_p128 = res["lam12_floor_p128"]
        if abs(r_raw["ari"] - r_p128["ari"]) > 0.005:
            tk = r_raw["true_k"]
            delta = r_p128["ari"] - r_raw["ari"]
            saved = "SAVED" if delta > 0 else "HURT"
            n_activated += 1
            print(
                f"    {saved:5s} {case_name:<40} TK={tk:>3}  "
                f"raw: K={r_raw['found_k']:>3} {r_raw['ari']:.3f}  "
                f"p128: K={r_p128['found_k']:>3} {r_p128['ari']:.3f}  "
                f"Δ={delta:+.3f}"
            )
    print(f"\n  Floor activated in {n_activated}/{n_cases} cases")

    # Cases where p128 differs from JL/8
    print("\n  Cases where p128 ≠ JL/8:\n")
    n_diff = 0
    for case_name, res in all_results:
        r_8th = res["lam12_floor_8th"]
        r_p128 = res["lam12_floor_p128"]
        if abs(r_8th["ari"] - r_p128["ari"]) > 0.005:
            tk = r_8th["true_k"]
            delta = r_p128["ari"] - r_8th["ari"]
            better = "p128+" if delta > 0 else "8th+"
            n_diff += 1
            print(
                f"    {better:5s} {case_name:<40} TK={tk:>3}  "
                f"8th: K={r_8th['found_k']:>3} {r_8th['ari']:.3f}  "
                f"p128: K={r_p128['found_k']:>3} {r_p128['ari']:.3f}  "
                f"Δ={delta:+.3f}"
            )
    print(f"\n  p128 differs from JL/8 in {n_diff}/{n_cases} cases")
