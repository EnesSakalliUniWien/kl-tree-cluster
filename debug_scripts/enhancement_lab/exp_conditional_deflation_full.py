...
This benchmark compares df-mismatch heuristics only. The current
...
def main() -> None:
    ...
    # Pairwise diff: df_mismatch_0.3 vs global
    if "global" in STRATEGIES and "df_mismatch_0.3" in STRATEGIES:
        print("\n--- df_mismatch_0.3 vs global ---")
        improvements = []
        regressions = []
        for c in case_names:
            if c not in all_results:
                continue
            if "global" not in all_results[c] or "df_mismatch_0.3" not in all_results[c]:
                continue
            a_base = all_results[c]["global"]["ari"]
            a_new = all_results[c]["df_mismatch_0.3"]["ari"]
            diff = a_new - a_base
            if diff > 0.005:
        ...
        for c, a_base, a_new, diff in sorted(improvements, key=lambda x: -x[3]):
            ...
            k_new = all_results[c]["df_mismatch_0.3"]["found_k"]
